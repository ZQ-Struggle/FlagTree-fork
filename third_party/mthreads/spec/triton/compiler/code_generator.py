...
class CodeGenerator(ast.NodeVisitor):
    ...
    def call_JitFunction(self, fn: JITFunction, args, kwargs, caller_context=None):
        ...
        if not self.module.has_function(fn_name):
            ...
            backend_supports_noinline = getattr(self.builder.options, "supports_noinline", True)
            generator = CodeGenerator(self.context, prototype, fn.get_capture_scope(), module=self.module, jit_fn=fn,
                                      ...
                                      noinline=fn.noinline and backend_supports_noinline, file_name=file_name,
                                      begin_line=begin_line, options=self.builder.options,
    ...
    def visit_Call(self, node):
        ...
        tme_descriptor_fns = (
            getattr(language.core, "load_tensor_descriptor", None),
            getattr(language.core, "store_tensor_descriptor", None),
            getattr(language.core, "_experimental_descriptor_load", None),
            getattr(language.core, "_experimental_descriptor_store", None),
        )
        if fn in tme_descriptor_fns:
            self.uses_tme = True
...
def ast_to_ttir(fn, src, context, options, codegen_fns, module_map, module=None):
    ...
    module.uses_tme = generator.uses_tme
