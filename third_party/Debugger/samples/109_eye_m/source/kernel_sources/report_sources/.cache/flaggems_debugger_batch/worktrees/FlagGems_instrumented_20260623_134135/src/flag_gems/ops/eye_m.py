...
@libentry()
@triton.jit
def eye_kernel(out_ptr, N, M, BLOCK_i: tl.constexpr, BLOCK_j: tl.constexpr):
...
def eye_m(n, m, *, dtype=None, layout=torch.strided, device=None, pin_memory=None):
    ...
    out = torch.empty((n, m), dtype=dtype, device=device, layout=layout, pin_memory=pin_memory)
    BLOCK_SIZE = 32
    grid = (triton.cdiv(n, BLOCK_SIZE), triton.cdiv(m, BLOCK_SIZE))
    with torch_device_fn.device(device):
        eye_kernel[grid](out, n, m, BLOCK_SIZE, BLOCK_SIZE)
    return out
