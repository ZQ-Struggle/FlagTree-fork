...
@libentry()
@triton.jit
def log_softmax_kernel(output_ptr, input_ptr, M, N, K, BLOCK_M: tl.constexpr=8, BLOCK_N: tl.constexpr=256):
...
@triton.jit
def log_softmax_backward_kernel(out_ptr, out_grad_ptr, in_grad_ptr, M, N, K, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr):
    pid_m = ext.program_id(0)
    pid_k = ext.program_id(1)
    tl.debug_collect_start(level=1, addr_level=1)
    m_offset = pid_m * BLOCK_M + tl.arange(0, BLOCK_M)
...
def log_softmax_out(self, dim, half_to_float=False, *, out):
    ...
    if out.dtype != dtype:
        raise RuntimeError(f'_log_softmax.out: expected out dtype {dtype}, got {out.dtype}')
    K = inp.numel() // M // N
    grid = lambda meta: (triton.cdiv(M, meta['BLOCK_M']), K)
    with torch_device_fn.device(inp.device):
        log_softmax_kernel[grid](out, inp, M, N, K, num_warps=8)
    return out
...
def log_softmax_backward_out(grad_output, output, dim, input_dtype, *, out):
    ...
    if out.dtype != input_dtype:
        raise RuntimeError(f'_log_softmax_backward_data.out: expected out dtype {input_dtype}, got {out.dtype}')
    K = output.numel() // M // N
    grid = lambda meta: (triton.cdiv(M, meta['BLOCK_M']), K)
    with torch_device_fn.device(out.device):
        log_softmax_backward_kernel[grid](output, grad_output, out, M, N, K)
    return out
