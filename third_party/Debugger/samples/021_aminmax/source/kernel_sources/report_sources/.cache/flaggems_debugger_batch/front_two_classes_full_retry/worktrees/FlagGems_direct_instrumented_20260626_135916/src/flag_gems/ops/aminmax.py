...
@libentry()
@triton.jit
def aminmax_kernel_1(inp, min_out, max_out, M, BLOCK_SIZE: tl.constexpr):
...
@libentry()
@triton.jit
def aminmax_kernel_2(min_inp, max_inp, min_out, max_out, mid_size, BLOCK_MID: tl.constexpr):
    tl.debug_collect_start(level=1, addr_level=1)
...
@triton.jit
def aminmax_kernel(inp, min_out, max_out, M, N, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr):
...
def aminmax(inp, dim=None, keepdim=False, *, out=None):
    ...
    if dim is None:
        ...
        with torch_device_fn.device(inp.device):
            aminmax_kernel_1[mid_size, 1](inp, min_mid, max_mid, M, block_size)
            aminmax_kernel_2[1, 1](min_mid, max_mid, min_out, max_out, mid_size, block_mid)
