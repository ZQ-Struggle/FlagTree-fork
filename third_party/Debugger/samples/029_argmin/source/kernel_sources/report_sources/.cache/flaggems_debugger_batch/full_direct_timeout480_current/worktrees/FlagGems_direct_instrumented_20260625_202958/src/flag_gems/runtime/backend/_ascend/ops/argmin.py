...
@libentry()
@triton.jit
def argmin_kernel_1(inp, mid_value, mid_index, M, BLOCK_SIZE: tl.constexpr):
...
@libentry()
@triton.autotune(configs=[triton.Config({'BLOCK_M': 1, 'BLOCK_N': 512}), triton.Config({'BLOCK_M': 4, 'BLOCK_N': 256}), triton.Config({'BLOCK_M': 8, 'BLOCK_N': 128})], key=['M', 'N', 'K'])
@triton.jit
def argmin_kernel(inp, out_index, M, N, K, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr):
...
def argmin(inp, dim=None, keepdim=False, *, dtype=None):
    ...
    if dim is None:
        ...
        with torch_device_fn.device(inp.device):
            argmin_kernel_1[mid_size, 1, 1](inp, mid_value, mid_index, M, block_size)
        ...
        with torch_device_fn.device(inp.device):
            argmin_kernel[grid](inp, out_index, M, N, K)
        return out_index
