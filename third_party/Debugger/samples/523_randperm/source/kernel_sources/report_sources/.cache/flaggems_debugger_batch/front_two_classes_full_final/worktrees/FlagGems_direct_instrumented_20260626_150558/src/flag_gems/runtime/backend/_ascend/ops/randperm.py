...
@libentry()
@triton.jit
def bitonic_sortbykey_kernel(y_ptr, index_ptr, chunk_x, chunk_index, N: tl.constexpr, BLOCK_SIZE: tl.constexpr, DESCENDING: tl.constexpr):
    ...
    sorted_chunk_x, sorted_chunk_index = argsort(chunk_x_val, chunk_index_val, 0, descending=DESCENDING)
...
@libentry()
@triton.jit
def digit_hist_kernel(digit_hist, key, n_elements, bits_per_pass, bins, passes, bit_mask, bins_segment, BLOCK_SIZE: tl.constexpr):
...
@libentry()
@triton.autotune(configs=runtime.get_tuned_config('randperm'), key=['n_elements'])
@triton.jit
def radix_sortbykey_scatter_kernel(key_out, value_out, key_in, value_in, digit_hist, d_lookback, n_elements, bit_offset, passes, p, num_portions, portion_size, portion_id, bit_mask, bins_segment, max_tiles_per_portion, bins: tl.constexpr, BLOCK_SIZE: tl.constexpr):
    ...
    tl.debug_collect_start(level=1, addr_level=1)
    key_offset = portion_id_i64 * portion_size + pid0.to(tl.int64) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    key_mask = key_offset < n_elements
    ...
    for s in range(bins_segment):
        ...
        tl.store(d_lookback + ((portion_id * passes + p) * max_tiles_per_portion + pid0) * bins + bin_id, partial_counter, cache_modifier='.cg')
        bin_offset = p * (bins + 1) + bin_id
        prefix_offsets = tl.load(digit_hist + bin_offset + portion_id * passes * (bins + 1))
        bk = pid0 - 1
        inc_sum = bin_of_bucket
        while bk >= 0:
            rd_lbk_offset = ((portion_id * passes + p) * max_tiles_per_portion + bk) * bins + bin_id
        ...
        tl.store(d_lookback + ((portion_id * passes + p) * max_tiles_per_portion + pid0) * bins + bin_id, global_counter, cache_modifier='.cg')
        inc_bucket_offset = prefix_offsets.to(tl.int64) + inc_sum.to(tl.int64)
        if last_block and portion_id < num_portions - 1:
            tl.store(digit_hist + bin_offset + (portion_id + 1) * passes * (bins + 1), inc_bucket_offset)
        global_offsets = inc_bucket_offset - bin_of_bucket.to(tl.int64) + key_block_rank.to(tl.int64)
...
def duplicate_keys_shuffle_kernel(value_in, n_elements, philox_seed, philox_offset, BLOCK_SIZE: tl.constexpr):
    pid0 = tl.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    ...
    c0 = (philox_offset & 4294967295).to(tl.int32)
    c1 = (philox_offset >> 32 & 4294967295).to(tl.int32)
...
def sort_by_key(key, value, valid_bits):
    ...
    if n_elements > 4:
        ...
        digit_hist_slice = torch.empty((passes, bins + 1, grid_hist[0]), dtype=hist_dtype, device=key.device)
        digit_hist = torch.empty((num_portions, passes, bins + 1), dtype=hist_dtype, device=key.device)
        d_lookback = torch.empty(num_portions * passes * bins * max_tiles_per_portion, dtype=torch.int32, device=key.device)
        ...
        with torch_device_fn.device(key.device):
            digit_hist_kernel[grid_hist](digit_hist_slice, key, n_elements, bits_per_pass, bins, passes, bit_mask, bins_per_sgement, BLOCK_SIZE)
        ...
        for p in range(passes):
            ...
            for portion_id in range(num_portions):
                ...
                with torch_device_fn.device(key.device):
                    radix_sortbykey_scatter_kernel[grid_scatter](k_out, v_out, k_in, v_in, digit_hist, d_lookback, n_elements, bit_offset, passes, p, num_portions, portion_size, portion_id, bit_mask, bins_per_sgement, max_tiles_per_portion, bins, BLOCK_SIZE)
            bit_offset += bits_per_pass
        ...
        with torch_device_fn.device(key.device):
            duplicate_keys_shuffle_kernel[grid_shuffle](v_out, n_elements, philox_seed, philox_offset, BLOCK_SIZE_SHUFFLE, num_warps=4)
        return v_out
    else:
        ...
        with torch_device_fn.device(key.device):
            bitonic_sortbykey_kernel[grid](k_out, v_out, key, value, n_elements, BLOCK_SIZE, False)
        return v_out

def randperm(n, *, generator=None, out=None, dtype=torch.int64, layout=torch.strided, device=None, requires_grad=False, pin_memory=False):
    ...
    rand_key = torch.randint(low=keymin, high=keymax, size=[n], dtype=key_dtype, device=device)
