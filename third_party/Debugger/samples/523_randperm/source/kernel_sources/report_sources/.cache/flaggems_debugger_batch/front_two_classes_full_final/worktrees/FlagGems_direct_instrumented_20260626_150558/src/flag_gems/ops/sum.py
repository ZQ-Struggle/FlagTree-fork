...
def sum_kernel_2(mid, out, mid_size, BLOCK_MID: tl.constexpr):
    tl.debug_collect_start(level=1, addr_level=1)
    if tl.constexpr(mid.dtype.element_ty == tl.float16) or tl.constexpr(mid.dtype.element_ty == tl.bfloat16):
        cdtype = tl.float32
    else:
...
@triton.jit
def sum_dim_kernel_non_inner(output_ptr, input_ptr, M, N, K, TILE_N: tl.constexpr, TILE_K: tl.constexpr, ONE_TILE_PER_CTA: tl.constexpr):
    tl.debug_collect_start(level=1, addr_level=1)
    if tl.constexpr(input_ptr.dtype.element_ty == tl.float16) or tl.constexpr(input_ptr.dtype.element_ty == tl.bfloat16):
        cdtype = tl.float32
    else:
...
@triton.jit
def sum_dim_kernel_inner(output_ptr, input_ptr, M, N, TILE_N: tl.constexpr, ONE_TILE_PER_CTA: tl.constexpr):
    tl.debug_collect_start(level=1, addr_level=1)
    if tl.constexpr(input_ptr.dtype.element_ty == tl.float16) or tl.constexpr(input_ptr.dtype.element_ty == tl.bfloat16):
        cdtype = tl.float32
    else:
...
@libentry()
@libtuner(configs=runtime.get_tuned_config('naive_reduction'), key=['M', 'N'])
@triton.jit
    ...
    if len(dim) == 1:
        ...
        with torch_device_fn.device(inp.device):
            if K > 1:
                ...
                sum_dim_kernel_non_inner[grid](out, inp, M, N, K)
            else:
                grid = (M, 1, 1)
                sum_dim_kernel_inner[grid](out, inp, M, N)
