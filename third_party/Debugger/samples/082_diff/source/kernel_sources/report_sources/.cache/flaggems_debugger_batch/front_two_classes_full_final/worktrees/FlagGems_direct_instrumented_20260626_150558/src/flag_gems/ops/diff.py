...
@libentry()
@libtuner(configs=runtime.get_tuned_config('diff_1d'), key=['N'])
@triton.jit
...
@libentry()
@libtuner(configs=runtime.get_tuned_config('diff'), key=['M', 'N'])
@triton.jit
def diff_kernel_2d(in_ptr, out_ptr, M, N, M_STRIDE_IN, M_STRIDE_OUT, BLOCK_M: tl.constexpr, BLOCK_DIFF: tl.constexpr):
...
def diff(input, n=1, dim=-1, prepend=None, append=None) -> torch.Tensor:
    ...
    def _launch(src, dst, in_stride_m, out_stride_m, n_bound):
        if is_1d:
            ...
            grid = lambda meta: (triton.cdiv(M, meta['BLOCK_M']), triton.cdiv(n_bound - 1, meta['BLOCK_DIFF']))
            with torch_device_fn.device(src.device):
