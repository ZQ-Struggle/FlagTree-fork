...
def adaptive_avg_pool2d(input: torch.Tensor, output_size):
    ...
    if out_h == 0 or out_w == 0 or in_h == 0 or (in_w == 0):
        return torch.empty((in_n, in_c, out_h, out_w), device=input.device, dtype=input.dtype)
    ...
    adaptive_avg_pool2d_kernel[grid](input, output, input.stride(0), input.stride(1), input.stride(2), input.stride(3), in_c, in_h, in_w, out_h, out_w)
    return output
