...
@libentry()
@triton.jit
def index_add_kernel(inp_ptr, out_ptr, index_ptr, src_ptr, M, N, alpha, inp_len, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr):
...
def index_add(inp, dim, index, src, alpha=1):
    ...
    with torch_device_fn.device(inp.device):
        index_add_kernel[grid](inp, out, index, src, M, N, alpha, inp_len, BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N)
    if dim != final_dim:
...
def index_add_(inp, dim, index, src, alpha=1):
    ...
    if dim != final_dim:
        ...
        grid = (triton.cdiv(M, BLOCK_M), triton.cdiv(N, BLOCK_N))
        with torch_device_fn.device(inp.device):
            index_add_kernel[grid](inp_work, out_work, index, src_work, M, N, alpha, inp_len, BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N)
        ...
        grid = (triton.cdiv(M, BLOCK_M), triton.cdiv(N, BLOCK_N))
        with torch_device_fn.device(inp.device):
            index_add_kernel[grid](inp_contig, out_contig, index, src, M, N, alpha, inp_len, BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N)
