...
@triton.jit
def round_kernel(x_ptr, out_ptr, n_elements, decimals: tl.constexpr, BLOCK_SIZE: tl.constexpr, IS_FP32: tl.constexpr, IS_FP16: tl.constexpr, IS_BF16: tl.constexpr):
...
def round_func(input, decimals=0):
    ...
    if not input.is_contiguous():
        raise ValueError('round Triton kernel currently supports only contiguous tensors.')
    ...
    with torch_device_fn.device(input.device):
        round_kernel[grid](input, output, n_elements, decimals, BLOCK_SIZE=BLOCK_SIZE, IS_FP32=IS_FP32, IS_FP16=IS_FP16, IS_BF16=IS_BF16)
    return output
...
def round_out(input, *, decimals=0, out=None):
    ...
    grid = lambda meta: (triton.cdiv(n_elements, meta['BLOCK_SIZE']),)
    with torch_device_fn.device(input.device):
        round_kernel[grid](input, out, n_elements, decimals, BLOCK_SIZE=BLOCK_SIZE, IS_FP32=IS_FP32, IS_FP16=IS_FP16, IS_BF16=IS_BF16)
    return out
...
def round_(input, *, decimals=0):
    ...
    grid = lambda meta: (triton.cdiv(n_elements, meta['BLOCK_SIZE']),)
    with torch_device_fn.device(input.device):
        round_kernel[grid](input, input, n_elements, decimals, BLOCK_SIZE=BLOCK_SIZE, IS_FP32=IS_FP32, IS_FP16=IS_FP16, IS_BF16=IS_BF16)
    return input
