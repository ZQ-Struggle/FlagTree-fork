...
def copy_func_wrapper_rank_2(in0: Union[torch.Tensor, StridedBuffer], /, *, out0: Union[torch.Tensor, StridedBuffer]): 
...
@libentry()
@triton.jit
def copy_func_kernel_rank_2(
    in0_ptr: tl.tensor, # of tl.pointer_type
    out0_ptr: tl.tensor, # of tl.pointer_type
    in0_stride0: tl.constexpr, in0_stride1: tl.constexpr, # strides for in0
    out0_stride0: tl.constexpr, out0_stride1: tl.constexpr, # strides for out0
    s0, s1, # task_space
    num_tasks,
    tiles_per_cta: int,
    ...
    if one_tile_per_cta: # monolitic kernel style
        ...
        # loads
        in0 = tl.load(in0_ptr + offsets0[:, None] * in0_stride0 + offsets1[None, :] * in0_stride1, mask=mask).to(in0_ptr.type.element_ty)

        # compute
        out0 = copy_func(in0)

        in0 = tl.store(out0_ptr + offsets0[:, None] * out0_stride0 + offsets1[None, :] * out0_stride1, out0, mask=mask)
