...
def pool2d_output_size(in_size: int, kernel_size: int, stride: int, padding: int, dilation: int, ceil_mode: bool=False) -> int:
...
@triton.jit
def avg_pool2d_forward_kernel(input_ptr, output_ptr, in_stride_n, in_stride_c, in_stride_h, in_stride_w, in_c, in_h, in_w, out_h, out_w, kernel_h: tl.constexpr, kernel_w: tl.constexpr, stride_h: tl.constexpr, stride_w: tl.constexpr, padding_h: tl.constexpr, padding_w: tl.constexpr, dilation_h: tl.constexpr, dilation_w: tl.constexpr, COUNT_INCLUDE_PAD: tl.constexpr, divisor_override, BLOCK_H: tl.constexpr, BLOCK_W: tl.constexpr):
...
@libentry()
@triton.autotune(configs=[triton.Config({'BLOCK_H': 16, 'BLOCK_W': 16}, num_stages=4, num_warps=4), triton.Config({'BLOCK_H': 32, 'BLOCK_W': 16}, num_stages=3, num_warps=4), triton.Config({'BLOCK_H': 16, 'BLOCK_W': 32}, num_stages=3, num_warps=4), triton.Config({'BLOCK_H': 32, 'BLOCK_W': 32}, num_stages=2, num_warps=8), triton.Config({'BLOCK_H': 64, 'BLOCK_W': 32}, num_stages=2, num_warps=8), triton.Config({'BLOCK_H': 32, 'BLOCK_W': 64}, num_stages=2, num_warps=8)], key=['in_h', 'in_w', 'kernel_h', 'kernel_w', 'stride_h', 'stride_w'])
@triton.jit
def avg_pool2d_backward_kernel(grad_output_ptr, grad_input_ptr, in_c, in_h, in_w, out_h, out_w, in_stride_n, in_stride_c, in_stride_h, in_stride_w, out_stride_n, out_stride_c, out_stride_h, out_stride_w, kernel_h: tl.constexpr, kernel_w: tl.constexpr, stride_h: tl.constexpr, stride_w: tl.constexpr, padding_h: tl.constexpr, padding_w: tl.constexpr, dilation_h: tl.constexpr, dilation_w: tl.constexpr, COUNT_INCLUDE_PAD: tl.constexpr, divisor_override, BLOCK_H: tl.constexpr, BLOCK_W: tl.constexpr):
    pid_nc = tl.program_id(0)
    pid_hw = tl.program_id(1)
    tl.debug_collect_start(level=1, addr_level=1)
            ...
            if divisor_override != 0:
                ...
                for kh_count in range(0, kernel_h):
                    for kw_count in range(0, kernel_w):
                        ...
                        is_valid = (h_in_for_count >= 0) & (h_in_for_count < in_h) & (w_in_for_count >= 0) & (w_in_for_count < in_w)
                        count += is_valid.to(tl.int32)
                divisor = count.to(tl.float32)
            divisor = tl.where(divisor == 0, 1.0, divisor)
            grad_out_ptr = grad_output_base_ptr + h_out * out_stride_h + w_out * out_stride_w
            grad_out_val = tl.load(grad_out_ptr, mask=out_mask, other=0.0)
            grad_acc += tl.where(out_mask, grad_out_val / divisor, 0.0)
    grad_input_store_ptr = grad_input_block_ptr + h_in_offsets[:, None] * in_stride_h + w_in_offsets[None, :] * in_stride_w
...
def avg_pool2d(input: torch.Tensor, kernel_size, stride=None, padding=0, ceil_mode=False, count_include_pad=True, divisor_override=None):
    logger.debug('GEMS AVG_POOL2D FORWARD')
    if divisor_override is not None and divisor_override == 0:
        raise ValueError('divisor_override cannot be zero')
    input = input.contiguous()
    kernel_h, kernel_w, stride_h, stride_w, padding_h, padding_w = _parse_pool_params(kernel_size, stride, padding)
    dilation_h, dilation_w = (1, 1)
    in_n, in_c, in_h, in_w = input.shape
    out_h = pool2d_output_size(in_h, kernel_h, stride_h, padding_h, dilation_h, ceil_mode)
    out_w = pool2d_output_size(in_w, kernel_w, stride_w, padding_w, dilation_w, ceil_mode)
...
def avg_pool2d_backward(grad_output: torch.Tensor, input: torch.Tensor, kernel_size, stride, padding, ceil_mode, count_include_pad, divisor_override):
    ...
    grid = lambda meta: (in_n * in_c, triton.cdiv(in_h, meta['BLOCK_H']) * triton.cdiv(in_w, meta['BLOCK_W']))
