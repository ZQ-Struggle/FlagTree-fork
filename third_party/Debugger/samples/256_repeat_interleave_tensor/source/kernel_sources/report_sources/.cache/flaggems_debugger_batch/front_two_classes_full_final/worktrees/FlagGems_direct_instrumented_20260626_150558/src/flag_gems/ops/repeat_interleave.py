...
def repeat_interleave_self_int(inp, repeats, dim=None, *, output_size=None):
    ...
    if output_size is not None and output_size != output_shape[dim]:
        raise RuntimeError('repeat_interleave: Invalid output_size, expected {} but got {}'.format(output_shape[dim], output_size))
...
@triton.jit
def repeat_interleave_tensor_kernel(repeats_ptr, cumsum_ptr, out_ptr, size, BLOCK_SIZE: tl.constexpr):
...
def repeat_interleave_tensor(repeats, *, output_size=None):
    ...
    grid = (size,)
    BLOCK_SIZE = 32
    repeat_interleave_tensor_kernel[grid](repeats, cumsum, out, size, BLOCK_SIZE=BLOCK_SIZE, num_warps=1)
    return out
...
def repeat_interleave_self_tensor(inp, repeats, dim=None, *, output_size=None):
    ...
    if repeats.ndim == 0 or (repeats.ndim == 1 and repeats.size(0) == 1):
        return repeat_interleave_self_int(inp, repeats.item(), dim=dim, output_size=output_size)
    elif repeats.ndim > 1:
    ...
    if repeats.size(0) != inp_shape[dim]:
        raise RuntimeError('repeats must have the same size as input along dim, but got                 repeats.size(0) = {} and input.size({}) = {}'.format(repeats.size(0), dim, inp_shape[dim]))
