...
def addmv_kernel(A, B, Inp, Out, N, M, alpha, beta, stride_an, stride_am, stride_bm, stride_in, stride_outn, BLOCK_N: tl.constexpr, BLOCK_M: tl.constexpr):
...
def addmv(self, mat, vec, *, beta=1, alpha=1):
    ...
    with torch_device_fn.device(mat.device):
        addmv_kernel[grid](mat, vec, self, out, N, M, alpha, beta, mat.stride(0), mat.stride(1), vec.stride(0), self.stride(0), out.stride(0))
    return out
