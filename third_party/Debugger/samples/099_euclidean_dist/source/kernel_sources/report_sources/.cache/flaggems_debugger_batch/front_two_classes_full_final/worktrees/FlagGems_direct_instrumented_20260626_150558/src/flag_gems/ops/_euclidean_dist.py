...
@libentry()
@triton.jit
def _euclidean_dist_kernel(x1_ptr, x2_ptr, output_ptr, N, M, D, stride_x1, stride_x2, stride_out, BLOCK_D: tl.constexpr):
...
def _euclidean_dist(x1, x2):
    ...
    with torch_device_fn.device(x1.device):
        grid = (N, M)
        _euclidean_dist_kernel[grid](x1, x2, output, N, M, D, x1.stride(0), x2.stride(0), output.stride(0), BLOCK_D=BLOCK_D)
    return output
