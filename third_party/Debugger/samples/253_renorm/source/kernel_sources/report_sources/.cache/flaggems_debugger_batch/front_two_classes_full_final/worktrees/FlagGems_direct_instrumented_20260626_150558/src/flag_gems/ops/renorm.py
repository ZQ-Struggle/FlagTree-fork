...
@libentry()
@triton.jit
def renorm_kernel_norms(X, norms_out, M, N, p_val, BLOCK_SIZE: tl.constexpr):
    """Kernel to compute p-norms for each sub-tensor (one per row when dim=0)."""
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    if tl.constexpr(X.dtype.element_ty == tl.float16) or tl.constexpr(X.dtype.element_ty == tl.bfloat16):
        cdtype = tl.float32
    else:
...
@libentry()
@triton.jit
def renorm_kernel_scale(X, norms_in, Y, M, N, p_val, maxnorm, BLOCK_SIZE: tl.constexpr):
...
def renorm(input, p, dim, maxnorm):
    ...
    if dim == 0:
        ...
        with torch_device_fn.device(input.device):
            renorm_kernel_norms[grid](input, norms, M, N, p, BLOCK_SIZE=BLOCK)
        output = torch.empty_like(input)
        with torch_device_fn.device(input.device):
            renorm_kernel_scale[grid](input, norms, output, M, N, p, maxnorm, BLOCK_SIZE=BLOCK)
        return output
    else:
        ndim = input.ndim
...
def renorm_(input, p, dim, maxnorm):
    ...
    if dim == 0:
        ...
        with torch_device_fn.device(input.device):
            renorm_kernel_norms[grid](input, norms, M, N, p, BLOCK_SIZE=BLOCK)
        with torch_device_fn.device(input.device):
            renorm_kernel_scale[grid](input, norms, input, M, N, p, maxnorm, BLOCK_SIZE=BLOCK)
        return input
    else:
        ndim = input.ndim
        perm = list(range(ndim))
