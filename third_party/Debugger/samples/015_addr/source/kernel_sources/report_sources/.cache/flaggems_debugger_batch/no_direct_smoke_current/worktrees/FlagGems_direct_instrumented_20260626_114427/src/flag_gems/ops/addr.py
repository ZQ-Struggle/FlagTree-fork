...
def addr_kernel(input_ptr, vec1_ptr, vec2_ptr, output_ptr, beta, alpha, M, N, stride_input_m, stride_input_n, stride_vec1, stride_vec2, stride_output_m, stride_output_n, BLOCK_SIZE_M: tl.constexpr, BLOCK_SIZE_N: tl.constexpr):
...
def addr(input, vec1, vec2, *, beta=1, alpha=1):
    ...
    try:
        ...
        raise ValueError(f'addr: input tensor of shape {input.shape} cannot be broadcast to output shape {output_shape}')
    out = torch.empty(output_shape, device=input.device, dtype=input.dtype)
    BLOCK_SIZE_M = 32
    BLOCK_SIZE_N = 32
    grid = lambda META: (triton.cdiv(M, BLOCK_SIZE_M), triton.cdiv(N, BLOCK_SIZE_N))
    with torch_device_fn.device(input.device):
        addr_kernel[grid](input_broadcasted, vec1, vec2, out, beta, alpha, M, N, input_broadcasted.stride(0), input_broadcasted.stride(1), vec1.stride(0), vec2.stride(0), out.stride(0), out.stride(1), BLOCK_SIZE_M=BLOCK_SIZE_M, BLOCK_SIZE_N=BLOCK_SIZE_N)
    return out
