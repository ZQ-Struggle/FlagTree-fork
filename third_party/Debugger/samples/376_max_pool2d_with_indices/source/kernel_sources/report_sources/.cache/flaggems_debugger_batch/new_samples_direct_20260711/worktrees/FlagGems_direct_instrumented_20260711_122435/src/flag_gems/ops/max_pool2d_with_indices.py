...
def max_pool2d_output_size(in_size: int, kernel_size: int, stride: int, padding: int, dilation: int, ceil_mode: bool=False) -> int:
...
@triton.jit
def max_pool2d_forward_kernel(input_ptr, output_ptr, indices_ptr, in_stride_n, in_stride_c, in_stride_h, in_stride_w, in_c, in_h, in_w, out_h, out_w, kernel_h: tl.constexpr, kernel_w: tl.constexpr, stride_h: tl.constexpr, stride_w: tl.constexpr, padding_h: tl.constexpr, padding_w: tl.constexpr, dilation_h: tl.constexpr, dilation_w: tl.constexpr, BLOCK_H: tl.constexpr, BLOCK_W: tl.constexpr):
...
@libentry()
@triton.autotune(configs=[triton.Config({'BLOCK_IN_H': 16, 'BLOCK_IN_W': 16}, num_warps=4), triton.Config({'BLOCK_IN_H': 32, 'BLOCK_IN_W': 8}, num_warps=4), triton.Config({'BLOCK_IN_H': 8, 'BLOCK_IN_W': 32}, num_warps=4), triton.Config({'BLOCK_IN_H': 32, 'BLOCK_IN_W': 32}, num_warps=8), triton.Config({'BLOCK_IN_H': 16, 'BLOCK_IN_W': 64}, num_warps=8), triton.Config({'BLOCK_IN_H': 64, 'BLOCK_IN_W': 16}, num_warps=8)], key=['in_h', 'in_w', 'kernel_h', 'kernel_w', 'stride_h', 'stride_w'])
@triton.jit
def max_pool2d_backward_kernel(grad_output_ptr, indices_ptr, grad_input_ptr, in_h, in_w, out_h, out_w, out_stride_nc, out_stride_h, out_stride_w, kernel_h: tl.constexpr, kernel_w: tl.constexpr, stride_h: tl.constexpr, stride_w: tl.constexpr, padding_h: tl.constexpr, padding_w: tl.constexpr, dilation_h: tl.constexpr, dilation_w: tl.constexpr, BLOCK_IN_H: tl.constexpr, BLOCK_IN_W: tl.constexpr):
    ...
    for kh in tl.static_range(0, kernel_h):
        for kw in tl.static_range(0, kernel_w):
            ...
            valid_map_mask = (numerator_h % stride_h == 0) & (numerator_w % stride_w == 0)
            h_out = numerator_h // stride_h
            w_out = numerator_w // stride_w
            out_bounds_mask = (h_out >= 0) & (h_out < out_h) & (w_out >= 0) & (w_out < out_w)
            ...
            indices_block = tl.load(indices_base_ptr + out_offsets, mask=load_mask, other=-1)
            match_mask = indices_block == current_input_flat_idx
            grad_block = tl.load(grad_output_base_ptr + out_offsets, mask=match_mask, other=0.0)
            grad_acc += grad_block
...
def _parse_pool_params(kernel_size, stride, padding, dilation):
    ...
    if stride_h <= 0 or stride_w <= 0:
        raise ValueError(f'stride must be positive, but got stride=({stride_h}, {stride_w})')
    if padding_h < 0 or padding_w < 0:
        raise ValueError(f'padding must be non-negative, but got padding=({padding_h}, {padding_w})')
    if dilation_h <= 0 or dilation_w <= 0:
        raise ValueError(f'dilation must be positive, but got dilation=({dilation_h}, {dilation_w})')
    return (kernel_h, kernel_w, stride_h, stride_w, padding_h, padding_w, dilation_h, dilation_w)

def max_pool2d_with_indices(input: torch.Tensor, kernel_size, stride=None, padding=0, dilation=1, ceil_mode=False):
    logger.debug('GEMS MAX_POOL2D_WITH_INDICES FORWARD')
    input = input.contiguous()
    params = _parse_pool_params(kernel_size, stride, padding, dilation)
    kernel_h, kernel_w, stride_h, stride_w, padding_h, padding_w, dilation_h, dilation_w = params
    in_n, in_c, in_h, in_w = input.shape
    out_h = max_pool2d_output_size(in_h, kernel_h, stride_h, padding_h, dilation_h, ceil_mode)
    out_w = max_pool2d_output_size(in_w, kernel_w, stride_w, padding_w, dilation_w, ceil_mode)
    ...
    grid = lambda meta: (in_n * in_c, triton.cdiv(out_h, meta['BLOCK_H']) * triton.cdiv(out_w, meta['BLOCK_W']))
    max_pool2d_forward_kernel[grid](input, output, indices, input.stride(0), input.stride(1), input.stride(2), input.stride(3), in_c, in_h, in_w, out_h, out_w, kernel_h, kernel_w, stride_h, stride_w, padding_h, padding_w, dilation_h, dilation_w)
    return (output, indices)

def max_pool2d_backward(grad_output: torch.Tensor, input: torch.Tensor, indices: torch.Tensor, kernel_size, stride, padding, dilation, ceil_mode):
    ...
    grid = lambda meta: (in_n * in_c, triton.cdiv(in_h, meta['BLOCK_IN_H']) * triton.cdiv(in_w, meta['BLOCK_IN_W']))
    out_stride_nc = out_h * out_w
    out_stride_h = out_w
    out_stride_w = 1
    max_pool2d_backward_kernel[grid](grad_output, indices, grad_input, in_h, in_w, out_h, out_w, out_stride_nc, out_stride_h, out_stride_w, kernel_h, kernel_w, stride_h, stride_w, padding_h, padding_w, dilation_h, dilation_w)
    return grad_input.to(grad_output.dtype)
