...
@libentry()
@libtuner(configs=runtime.get_tuned_config('baddbmm'), key=['M', 'N', 'K'], strategy=['align32', 'align32', 'align32'], warmup=5, rep=10)
@triton.heuristics(_hcu.HEURISTICS_CONFIGS['baddbmm'])
@triton.jit(do_not_specialize=['alpha', 'beta'])
def baddbmm_kernel(A, B, O, bias, alpha, beta, M, N, K, TILE_M: tl.constexpr, TILE_N: tl.constexpr, TILE_K: tl.constexpr, GROUP_M: tl.constexpr, DIVISIBLE_M: tl.constexpr, DIVISIBLE_N: tl.constexpr, DIVISIBLE_K: tl.constexpr, bias_batch_stride: tl.constexpr, bias_M_stride: tl.constexpr, bias_N_stride: tl.constexpr):
...
class BaddbmmFunction(torch.autograd.Function):
    ...
    def forward(ctx, bias, A, B, beta, alpha):
        ...
        grid = lambda meta: (triton.cdiv(meta['M'], meta['TILE_M']), triton.cdiv(meta['N'], meta['TILE_N']), batch)
        with torch_device_fn.device(A.device):
            baddbmm_kernel[grid](A, B, out, bbias, alpha, beta, M, N, K, bias_batch_stride=bias_batch_stride, bias_M_stride=bias_M_stride, bias_N_stride=bias_N_stride)
        return out
...
def baddbmm(bias, A, B, beta=1.0, alpha=1.0):
    return BaddbmmFunction.apply(bias.contiguous(), A.contiguous(), B.contiguous(), beta, alpha)
