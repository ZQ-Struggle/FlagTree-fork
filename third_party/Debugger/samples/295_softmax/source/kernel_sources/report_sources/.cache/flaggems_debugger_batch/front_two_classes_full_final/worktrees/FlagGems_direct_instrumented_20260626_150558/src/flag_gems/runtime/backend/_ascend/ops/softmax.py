...
@triton.jit
def softmax_kernel_non_inner(output_ptr, input_ptr, M, N, K, TILE_N: tl.constexpr, TILE_K: tl.constexpr, ONE_TILE_PER_CTA: tl.constexpr):
...
@triton.jit
def softmax_kernel_inner(output_ptr, input_ptr, M, N, TILE_N: tl.constexpr, ONE_TILE_PER_CTA: tl.constexpr):
    pid_m = ext.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    if ONE_TILE_PER_CTA:
        n_offsets = tl.arange(0, TILE_N)
        ...
        for start_n in range(0, TILE_N, TILE_N):
            n_offsets = previous_multiple - start_n + tl.arange(0, TILE_N)
            mask = n_offsets < N
            inp = tl.load(input_ptr + n_offsets, mask=mask, other=-float('inf'), eviction_policy='evict_first')
            o = tl.exp(inp - m) / z
...
@libentry()
@triton.autotune(configs=runtime.get_tuned_config('softmax_non_inner'), key=['M', 'N', 'K'])
@triton.heuristics(_hcu.HEURISTICS_CONFIGS['softmax_backward_non_inner'])
@triton.jit
def softmax_backward_kernel_non_inner(out_ptr, out_grad_ptr, in_grad_ptr, M, N, K, TILE_N: tl.constexpr, TILE_K: tl.constexpr, ONE_TILE_PER_CTA: tl.constexpr):
    pid_m = ext.program_id(0)
    pid_k = ext.program_id(1)
    tl.debug_collect_start(level=1, addr_level=1)
...
@libentry()
@triton.autotune(configs=runtime.get_tuned_config('softmax_inner'), key=['M', 'N'])
@triton.heuristics(values=_hcu.HEURISTICS_CONFIGS['softmax_backward_inner'])
@triton.jit
def softmax_backward_kernel_inner(out_ptr, out_grad_ptr, in_grad_ptr, M, N, TILE_M: tl.constexpr, TILE_N: tl.constexpr, ONE_TILE_PER_CTA: tl.constexpr):
    pid_m = ext.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    ...
    if ONE_TILE_PER_CTA:
        ...
        for off in range(0, N, TILE_N):
            ...
            offsets = m_offsets[:, None] * N + n_offsets
            mask = (m_offsets[:, None] < M) & (n_offsets < N)
            out_tile = tl.load(out_ptr + offsets, mask=mask, eviction_policy='evict_last').to(tl.float32)
            out_grad_tile = tl.load(out_grad_ptr + offsets, mask=mask).to(tl.float32)
        ...
        for off in range(0, N, TILE_N):
            ...
            offsets = m_offsets[:, None] * N + n_offsets
            mask = (m_offsets[:, None] < M) & (n_offsets < N)
            out_tile = tl.load(out_ptr + offsets, mask=mask, eviction_policy='evict_first').to(tl.float32)
            out_grad_tile = tl.load(out_grad_ptr + offsets, mask=mask).to(tl.float32)
            in_grad_tile = out_tile * (out_grad_tile - scale[:, None])
            tl.store(in_grad_ptr + offsets, in_grad_tile, mask=mask)
    tl.debug_collect_end()
...
def softmax_backward(grad_output, output, dim, input_dtype):
    ...
    with torch_device_fn.device(in_grad.device):
        if K > 1:
            grid = lambda meta: (M, triton.cdiv(K, meta['TILE_K']), 1)
            softmax_backward_kernel_non_inner[grid](output, grad_output, in_grad, M, N, K)
        else:
...
def softmax_backward_out(grad_output, output, dim, input_dtype, *, grad_input):
    ...
    with torch_device_fn.device(grad_input.device):
        if K > 1:
            grid = lambda meta: (M, triton.cdiv(K, meta['TILE_K']), 1)
            softmax_backward_kernel_non_inner[grid](output, grad_output, grad_input, M, N, K)
        else:
            grid = lambda meta: (triton.cdiv(M, meta['TILE_M']), 1, 1)
