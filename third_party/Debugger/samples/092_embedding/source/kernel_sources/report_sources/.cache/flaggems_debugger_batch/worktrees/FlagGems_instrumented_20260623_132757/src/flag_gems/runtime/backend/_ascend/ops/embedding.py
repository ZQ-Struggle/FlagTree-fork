...
class Embedding(torch.autograd.Function):
    ...
    def backward(ctx, grad_outputs):
        ...
        grad_inputs = torch.zeros((ctx.num_weights, grad_outputs.shape[-1]), device=grad_outputs.device, dtype=torch.float32 if grad_outputs.dtype is torch.bfloat16 else grad_outputs.dtype)
        ...
        with torch_device_fn.device(grad_outputs.device):
            embedding_backward_kernel[ctx.M,](grad_inputs, grad_outputs, ctx.indices, ctx.padding_idx, HAS_PADDING_IDX, ctx.N, BLOCK_SIZE)
        if ctx.scale_grad_by_freq:
            with torch_device_fn.device(grad_outputs.device):
                embedding_grad_scale_kernel[ctx.M,](grad_inputs, indice_freq, ctx.num_weights, ctx.N, BLOCK_SIZE)
        return (grad_inputs.to(torch.bfloat16) if grad_outputs.dtype is torch.bfloat16 else grad_inputs, None, None, None, None)
