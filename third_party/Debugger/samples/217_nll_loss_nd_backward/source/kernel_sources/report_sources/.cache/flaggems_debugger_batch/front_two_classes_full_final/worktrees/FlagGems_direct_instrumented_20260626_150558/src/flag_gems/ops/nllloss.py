...
def nll_loss_forward_kernel(inp_ptr, tgt_ptr, wgt_ptr, out_ptr, ignore_index, N, C, reduction: tl.constexpr=1, BLOCK_N: tl.constexpr=128):
...
@libentry()
@triton.jit(do_not_specialize=['ignore_index'])
def nll_loss_backward_kernel(out_grad_ptr, tgt_ptr, wgt_ptr, inp_grad_ptr, ignore_index, total_weight, N, C, reduction: tl.constexpr=1, BLOCK_N: tl.constexpr=128):
    pid_n = tl.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets_n = pid_n * BLOCK_N + tl.arange(0, BLOCK_N)
    mask_n = offsets_n < N
...
@libentry()
@triton.jit(do_not_specialize=['ignore_index'])
def nll_loss2d_forward_kernel(inp_ptr, tgt_ptr, wgt_ptr, out_ptr, ignore_index, N, C, D, reduction: tl.constexpr=1, BLOCK_ND: tl.constexpr=128):
...
@libentry()
@triton.jit(do_not_specialize=['ignore_index'])
def nll_loss2d_backward_kernel(out_grad_ptr, tgt_ptr, wgt_ptr, inp_grad_ptr, ignore_index, total_weight, N, C, D, reduction: tl.constexpr=1, BLOCK_ND: tl.constexpr=128):
    pid_nd = tl.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offset_nd = pid_nd * BLOCK_ND + tl.arange(0, BLOCK_ND)
    offset_d = offset_nd % D
...
def nll_loss_forward(self, target, weight=None, reduction=1, ignore_index=-100):
    ...
    with torch_device_fn.device(self.device):
        nll_loss_forward_kernel[grid](self, target, weight, out, ignore_index, N, C, reduction)
    if reduction == 0:
        output = out
...
def nll_loss_backward(grad_output, self, target, weight=None, reduction=1, ignore_index=-100, total_weight=None):
    ...
    grid = lambda meta: (triton.cdiv(N, meta['BLOCK_N']),)
    with torch_device_fn.device(self.device):
        nll_loss_backward_kernel[grid](grad_output, target, weight, grad_input, ignore_index, total_weight, N, C, reduction)
    return grad_input
...
def nll_loss2d_forward(self, target, weight=None, reduction=1, ignore_index=-100):
    ...
    with torch_device_fn.device(self.device):
        nll_loss2d_forward_kernel[grid](self, target, weight, out, ignore_index, N, C, D, reduction)
    if reduction == 0:
        output = out
        total_weight = torch.empty([], dtype=self.dtype, device=self.device)
    elif reduction == 1:
...
def nll_loss2d_backward(grad_output, self, target, weight=None, reduction=1, ignore_index=-100, total_weight=None):
    ...
    grid = lambda meta: (triton.cdiv(N * D, meta['BLOCK_ND']),)
    with torch_device_fn.device(self.device):
        nll_loss2d_backward_kernel[grid](grad_output, target, weight, grad_input, ignore_index, total_weight, N, C, D, reduction)
    return grad_input
