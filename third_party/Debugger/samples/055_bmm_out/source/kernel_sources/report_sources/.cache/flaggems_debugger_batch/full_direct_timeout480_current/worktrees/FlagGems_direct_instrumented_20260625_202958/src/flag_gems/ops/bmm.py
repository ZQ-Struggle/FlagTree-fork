...
@libentry()
@libtuner(configs=runtime.get_tuned_config('bmm'), key=['M', 'N', 'K', 'stride_am', 'stride_bk'], strategy=['log', 'log', 'log', 'align32', 'align32'], flagtune_op_name='bmm', flagtune_expand_op_name='bmm', flagtune_pre_hook=None)
@triton.heuristics(runtime.get_heuristic_config('bmm'))
@triton.jit
def bmm_kernel(A, B, O, M, N, K, stride_ab, stride_am, stride_ak, stride_bb, stride_bk, stride_bn, stride_ob, stride_om, stride_on, TILE_M: tl.constexpr, TILE_N: tl.constexpr, TILE_K: tl.constexpr, GROUP_M: tl.constexpr, DIVISIBLE_M: tl.constexpr, DIVISIBLE_N: tl.constexpr, DIVISIBLE_K: tl.constexpr, IS_FP64: tl.constexpr=False):
...
def bmm(A, B):
    ...
    with torch_device_fn.device(A.device):
        bmm_kernel[grid_fn](A, B, out, M, N, K, A.stride(0), A.stride(1), A.stride(2), B.stride(0), B.stride(1), B.stride(2), out.stride(0), out.stride(1), out.stride(2), IS_FP64=A.dtype == torch.float64)
    return out
