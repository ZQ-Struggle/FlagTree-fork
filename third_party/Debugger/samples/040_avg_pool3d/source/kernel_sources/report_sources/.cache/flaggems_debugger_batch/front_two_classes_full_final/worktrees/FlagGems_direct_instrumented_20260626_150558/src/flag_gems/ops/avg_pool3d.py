...
def pool3d_output_size(in_size: int, kernel_size: int, stride: int, padding: int, dilation: int, ceil_mode: bool=False) -> int:
...
@libentry()
@triton.autotune(configs=[triton.Config({'BLOCK_H': 16, 'BLOCK_W': 16}, num_stages=4, num_warps=4), triton.Config({'BLOCK_H': 32, 'BLOCK_W': 16}, num_stages=3, num_warps=4), triton.Config({'BLOCK_H': 16, 'BLOCK_W': 32}, num_stages=3, num_warps=4), triton.Config({'BLOCK_H': 32, 'BLOCK_W': 32}, num_stages=2, num_warps=8), triton.Config({'BLOCK_H': 8, 'BLOCK_W': 8}, num_stages=5, num_warps=2), triton.Config({'BLOCK_H': 8, 'BLOCK_W': 16}, num_stages=5, num_warps=2), triton.Config({'BLOCK_H': 16, 'BLOCK_W': 8}, num_stages=5, num_warps=2), triton.Config({'BLOCK_H': 64, 'BLOCK_W': 16}, num_stages=2, num_warps=8), triton.Config({'BLOCK_H': 16, 'BLOCK_W': 64}, num_stages=2, num_warps=8)], key=['out_d', 'out_h', 'out_w', 'kernel_d', 'kernel_h', 'kernel_w'])
@triton.jit
def avg_pool3d_forward_kernel(input_ptr, output_ptr, in_stride_n, in_stride_c, in_stride_d, in_stride_h, in_stride_w, in_c, in_d, in_h, in_w, out_d, out_h, out_w, kernel_d: tl.constexpr, kernel_h: tl.constexpr, kernel_w: tl.constexpr, stride_d: tl.constexpr, stride_h: tl.constexpr, stride_w: tl.constexpr, padding_d: tl.constexpr, padding_h: tl.constexpr, padding_w: tl.constexpr, dilation_d: tl.constexpr, dilation_h: tl.constexpr, dilation_w: tl.constexpr, COUNT_INCLUDE_PAD: tl.constexpr, divisor_override, BLOCK_H: tl.constexpr, BLOCK_W: tl.constexpr):
    ...
    if divisor_override != 0:
        ...
        d_padded_count = tl.minimum(d_start_fwd + kernel_d, in_d + padding_d) - tl.maximum(d_start_fwd, -padding_d)
        d_padded_count = tl.maximum(d_padded_count, 0)
        h_start_fwd = h_out_offsets[:, None] * stride_h - padding_h
        h_padded_count = tl.minimum(h_start_fwd + kernel_h, in_h + padding_h) - tl.maximum(h_start_fwd, -padding_h)
        h_padded_count = tl.maximum(h_padded_count, 0)
        w_start_fwd = w_out_offsets[None, :] * stride_w - padding_w
        w_padded_count = tl.minimum(w_start_fwd + kernel_w, in_w + padding_w) - tl.maximum(w_start_fwd, -padding_w)
...
@libentry()
@triton.autotune(configs=[triton.Config({'BLOCK_H': 16, 'BLOCK_W': 16}, num_stages=4, num_warps=4), triton.Config({'BLOCK_H': 32, 'BLOCK_W': 16}, num_stages=3, num_warps=4), triton.Config({'BLOCK_H': 16, 'BLOCK_W': 32}, num_stages=3, num_warps=4), triton.Config({'BLOCK_H': 32, 'BLOCK_W': 32}, num_stages=2, num_warps=8), triton.Config({'BLOCK_H': 64, 'BLOCK_W': 32}, num_stages=2, num_warps=8), triton.Config({'BLOCK_H': 32, 'BLOCK_W': 64}, num_stages=2, num_warps=8)], key=['in_h', 'in_w', 'kernel_d', 'kernel_h', 'kernel_w'])
@triton.jit
def avg_pool3d_backward_kernel(grad_output_ptr, grad_input_ptr, in_c, in_d, in_h, in_w, out_d, out_h, out_w, in_stride_n, in_stride_c, in_stride_d, in_stride_h, in_stride_w, out_stride_n, out_stride_c, out_stride_d, out_stride_h, out_stride_w, kernel_d: tl.constexpr, kernel_h: tl.constexpr, kernel_w: tl.constexpr, stride_d: tl.constexpr, stride_h: tl.constexpr, stride_w: tl.constexpr, padding_d: tl.constexpr, padding_h: tl.constexpr, padding_w: tl.constexpr, COUNT_INCLUDE_PAD: tl.constexpr, divisor_override, BLOCK_H: tl.constexpr, BLOCK_W: tl.constexpr):
    pid_nc = tl.program_id(0)
    pid_dhw = tl.program_id(1)
    tl.debug_collect_start(level=1, addr_level=1)
    ...
    for kd in range(kernel_d):
        ...
        for kh in range(kernel_h):
            for kw in range(kernel_w):
                ...
                out_mask = d_out_valid & h_valid & w_valid & (h_out < out_h) & (w_out < out_w)
                if divisor_override != 0:
                    divisor = tl.full((BLOCK_H, BLOCK_W), divisor_override, dtype=tl.float32)
                elif COUNT_INCLUDE_PAD:
                    d_start_bwd = d_out * stride_d - padding_d
                    d_pc = tl.minimum(d_start_bwd + kernel_d, in_d + padding_d) - tl.maximum(d_start_bwd, -padding_d)
                    d_pc = tl.maximum(d_pc, 0)
                    h_start_bwd = h_out * stride_h - padding_h
                    h_pc = tl.minimum(h_start_bwd + kernel_h, in_h + padding_h) - tl.maximum(h_start_bwd, -padding_h)
                    h_pc = tl.maximum(h_pc, 0)
                    w_start_bwd = w_out * stride_w - padding_w
                    w_pc = tl.minimum(w_start_bwd + kernel_w, in_w + padding_w) - tl.maximum(w_start_bwd, -padding_w)
                ...
                grad_out_ptr = grad_output_base + d_out * out_stride_d + h_out * out_stride_h + w_out * out_stride_w
                grad_out_val = tl.load(grad_out_ptr, mask=out_mask, other=0.0)
                grad_acc += tl.where(out_mask, grad_out_val / divisor, 0.0)
    grad_input_store_ptr = grad_input_base + d_in_idx * in_stride_d + h_in_offsets[:, None] * in_stride_h + w_in_offsets[None, :] * in_stride_w
    in_write_mask = (h_in_offsets[:, None] < in_h) & (w_in_offsets[None, :] < in_w)
    tl.store(grad_input_store_ptr, grad_acc.to(grad_input_ptr.type.element_ty), mask=in_write_mask)
    tl.debug_collect_end()
...
def avg_pool3d(input: torch.Tensor, kernel_size, stride=None, padding=0, ceil_mode=False, count_include_pad=True, divisor_override=None):
    ...
    kernel_d, kernel_h, kernel_w, stride_d, stride_h, stride_w, padding_d, padding_h, padding_w = _parse_pool3d_params(kernel_size, stride, padding)
    ...
    if output.numel() == 0:
        return output
    grid = lambda meta: (in_n * in_c, out_d * triton.cdiv(out_h, meta['BLOCK_H']) * triton.cdiv(out_w, meta['BLOCK_W']))
    avg_pool3d_forward_kernel[grid](input, output, input.stride(0), input.stride(1), input.stride(2), input.stride(3), input.stride(4), in_c, in_d, in_h, in_w, out_d, out_h, out_w, kernel_d, kernel_h, kernel_w, stride_d, stride_h, stride_w, padding_d, padding_h, padding_w, dilation_d, dilation_h, dilation_w, COUNT_INCLUDE_PAD=count_include_pad, divisor_override=divisor_override if divisor_override is not None else 0.0)
    return output

def avg_pool3d_backward(grad_output: torch.Tensor, input: torch.Tensor, kernel_size, stride, padding, ceil_mode, count_include_pad, divisor_override):
    ...
    grid = lambda meta: (in_n * in_c, in_d * triton.cdiv(in_h, meta['BLOCK_H']) * triton.cdiv(in_w, meta['BLOCK_W']))
    avg_pool3d_backward_kernel[grid](grad_output, grad_input, in_c, in_d, in_h, in_w, out_d, out_h, out_w, grad_input.stride(0), grad_input.stride(1), grad_input.stride(2), grad_input.stride(3), grad_input.stride(4), grad_output.stride(0), grad_output.stride(1), grad_output.stride(2), grad_output.stride(3), grad_output.stride(4), kernel_d, kernel_h, kernel_w, stride_d, stride_h, stride_w, padding_d, padding_h, padding_w, COUNT_INCLUDE_PAD=count_include_pad, divisor_override=divisor_override if divisor_override is not None else 0.0)
    return grad_input
