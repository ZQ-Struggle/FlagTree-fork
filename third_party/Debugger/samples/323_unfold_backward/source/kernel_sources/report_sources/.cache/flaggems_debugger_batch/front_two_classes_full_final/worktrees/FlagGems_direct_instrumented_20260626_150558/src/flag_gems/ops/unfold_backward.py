...
@triton.jit
def _unfold_backward_kernel(grad_in_ptr, grad_out_ptr, numel_in, prod_after, L, size, step, D, inner_total, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
...
def unfold_backward(grad_in: torch.Tensor, input_sizes, dim: int, size: int, step: int) -> torch.Tensor:
    ...
    _unfold_backward_kernel[grid](grad_in, grad_out_f32, numel_in, prod_after, L, size, step, D, inner_total, BLOCK=BLOCK)
