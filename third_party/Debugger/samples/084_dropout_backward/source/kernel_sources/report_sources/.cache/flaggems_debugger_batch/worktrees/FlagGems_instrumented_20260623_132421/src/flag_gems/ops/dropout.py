...
def dropout_forward_kernel(X, Y, dropout_mask, N, p, philox_seed, philox_offset, BLOCK: tl.constexpr):
...
def dropout_backward_kernel(DY, DX, dropout_mask, N, scale, BLOCK: tl.constexpr):
    tl.debug_collect_start(level=1, addr_level=1)
...
def dropout(input, p, train=True):
    ...
    with torch_device_fn.device(device):
        ...
        dropout_forward_kernel[grid_fn](input, out, mask, N, p, philox_seed, philox_offset)
    return (out, mask)
