...
@libentry()
@triton.autotune(configs=runtime.get_tuned_config('vstack'), key=['max_tile_elems'])
@triton.jit
def vstack_kernel(itensor_ptr0, itensor_ptr1, itensor_ptr2, itensor_ptr3, output_ptr, local_row0, local_row1, local_row2, local_row3, exc_row_offset0, exc_row_offset1, exc_row_offset2, exc_row_offset3, total_row_offset, row_stride, max_tile_elems, BLOCK_SIZE: tl.constexpr):
    ...
    mask2 = tensor_idx == 2
    mask3 = tensor_idx == 3
    base_exc_row_idx = mask0 * exc_row_offset0 + mask1 * exc_row_offset1 + mask2 * exc_row_offset2 + mask3 * exc_row_offset3
    local_row = mask0 * local_row0 + mask1 * local_row1 + mask2 * local_row2 + mask3 * local_row3
...
def vstack(tensors: list):
    ...
    for tensor in tensors:
        assert tensor.device == device and tensor.dtype == dtype and (tensors[0].shape[1:] == tensor.shape[1:])
    ...
    for i in range(outer_iters):
        ...
        for j in range(4):
            ...
            if tensor_idx < num_tensors:
                ...
                empty_tensor = torch.empty(0, dtype=c_tensors[0].dtype, device=c_tensors[0].device)
        ...
        grid = lambda META: (triton.cdiv(max_tile_elems, META['BLOCK_SIZE']), scheduled_num_tensors)
        with torch_device_fn.device(c_tensors[0].device):
            vstack_kernel[grid](itensors[0], itensors[1], itensors[2], itensors[3], output, local_row[0], local_row[1], local_row[2], local_row[3], exclusive_row[0], exclusive_row[1], exclusive_row[2], exclusive_row[3], total_row_offset, row_stride, max_tile_elems)
