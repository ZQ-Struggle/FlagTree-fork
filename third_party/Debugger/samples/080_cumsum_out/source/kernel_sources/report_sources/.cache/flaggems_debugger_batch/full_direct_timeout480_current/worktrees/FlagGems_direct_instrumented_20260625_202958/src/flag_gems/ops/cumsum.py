...
def scan_part_sum_kernel(inp, out, partial_sum, n_elements, part_num, BLOCK_SIZE: tl.constexpr):
    ...
    if (tl.constexpr(inp_vals.dtype.is_int64()) or tl.constexpr(inp_vals.dtype.is_uint64())) or tl.constexpr(inp_vals.dtype.is_fp64()):
        inp_vals = inp_vals
...
@libentry()
@triton.jit(do_not_specialize=['n_elements', 'part_num'])
def add_base_sum_kernel(out, partial_sum, n_elements, part_num, BLOCK_SIZE: tl.constexpr):
    pid = ext.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offset = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offset < n_elements
...
def scan_part_sum_abc_kernel(inp, out, partial_sum, B, C, part_num, BLOCK_SIZE: tl.constexpr):
...
@libentry()
@triton.jit(do_not_specialize=['part_num'])
def add_base_sum_abc_kernel(out, partial_sum, B, C, part_num, BLOCK_SIZE: tl.constexpr):
    pid_a = ext.program_id(0)
    pid_b = ext.program_id(1)
    pid_c = ext.program_id(2)
    a_idx = pid_a
...
def reduce_then_scan_row(x, out, M, N, compute_dtype):
    if N <= 16384:
        ...
        reduce_then_scan_root_scan_kernel_row[M, 1, 1](x, out, N, TILE_SIZE, num_warps=num_warps)
        return out
    ...
    block_inclusive_prefix = torch.empty((M, num_ctas), dtype=compute_dtype, device=x.device)
    reduce_then_scan_block_sum_kernel_row[M, num_ctas, 1, 1](x, block_sums, N, tiles_per_cta, TILE_SIZE, num_warps=num_warps)
    reduce_then_scan_root_scan_kernel_row[M, 1, 1](block_sums, block_inclusive_prefix, num_ctas, ROOT_SCAN_TILE_SIZE, num_warps=num_warps)
    reduce_then_scan_block_scan_kernel_row[M, num_ctas, 1](x, block_inclusive_prefix, out, N, num_ctas, tiles_per_cta, TILE_SIZE, num_warps=num_warps)
    return out

@triton.jit
def reduce_then_scan_block_sum_kernel_row(in_ptr, block_sum_ptr, N, tiles_per_cta, TILE_SIZE: tl.constexpr):
    ...
    tl.store(block_sum_ptr + pid_m * num_programs_n + pid_n, block_sum, cache_modifier='.cg')
    tl.debug_collect_end()
...
@triton.jit
def reduce_then_scan_block_scan_kernel_row(in_ptr, previous_sum_ptr, out_ptr, N, num_tiles_n, tiles_per_cta, TILE_SIZE: tl.constexpr):
    pid_m = tl.program_id(0).to(tl.int64)
    pid_n = tl.program_id(1).to(tl.int64)
    block_offset = pid_n * (tiles_per_cta * TILE_SIZE)
    block_end = min(block_offset + tiles_per_cta * TILE_SIZE, N)
    acc_dtype: tl.constexpr = get_scan_accum_type(in_ptr.type.element_ty)
    tl.debug_collect_start(level=1, addr_level=1)
    prefix = tl.load(previous_sum_ptr + pid_m * num_tiles_n + pid_n - 1, mask=pid_n > 0, other=0).to(acc_dtype)
    for start in range(block_offset, block_end, TILE_SIZE):
        offsets = start + tl.arange(0, TILE_SIZE)
        mask = offsets < N
        ...
        prefix += tl.sum(x, 0)
        tl.store(out_ptr + pid_m * N + offsets, tile_scan, mask=mask, cache_modifier='.cg')
    tl.debug_collect_end()
...
@libentry()
@triton.jit(do_not_specialize=['r', 't', 'R', 'K', 'r_stride', 'out_r_stride'])
def block_cumsum_kernel(inp, out, sums, r, t, R, K, r_stride, k_stride, out_r_stride, out_k_stride, OUTPUT_SUMS: tl.constexpr, NORMALIZE: tl.constexpr, HAS_OUT_LAYOUT: tl.constexpr, TILE: tl.constexpr):
...
@libentry()
@triton.jit(do_not_specialize=['r', 't', 'R', 'K', 'r_stride', 'out_r_stride'])
def block_update_kernel(inp, base, rscale_ptr, out, r, t, R, K, r_stride, k_stride, out_r_stride, out_k_stride, rscale_stride, HAS_OUT_LAYOUT: tl.constexpr, TILE: tl.constexpr):
    gridx = ext.program_id(0).to(tl.int64)
    gridy = ext.program_id(1).to(tl.int64)
...
def normed_cumsum(inp, dim=-1):
    ...
    with torch_device_fn.device(inp.device.index):
        ...
        if n_chunks == 1:
            block_cumsum_kernel[grid](inp, out, 0, batch, n_tiles, n_rows, K, r_stride, k_stride, r_stride, k_stride, OUTPUT_SUMS=False, NORMALIZE=True, HAS_OUT_LAYOUT=False, TILE=TILE)
            return out
        if inp.dtype != torch.float64:
            acc_dtype = torch.float32
        sums = torch.empty((n_rows, n_chunks), dtype=acc_dtype, device=device.name)
        cumsums = torch.empty_like(sums)
        block_cumsum_kernel[grid](inp, out, sums, batch, n_tiles, n_rows, K, r_stride, k_stride, r_stride, k_stride, OUTPUT_SUMS=True, NORMALIZE=False, HAS_OUT_LAYOUT=False, TILE=TILE)
        block_cumsum_kernel[1, n_batch](sums, cumsums, 0, batch, 1, n_rows, n_chunks, n_chunks, 1, n_chunks, 1, OUTPUT_SUMS=False, NORMALIZE=False, HAS_OUT_LAYOUT=True, TILE=TILE)
        rscale = cumsums[..., -1]
        block_update_kernel[grid](out, cumsums - sums, rscale, out, batch, n_tiles, n_rows, K, r_stride, k_stride, r_stride, k_stride, n_chunks, HAS_OUT_LAYOUT=False, TILE=TILE)
        return out
