...
@libentry()
@triton.jit
def min_kernel_1(inp, mid, M, BLOCK_SIZE: tl.constexpr):
...
@libentry()
@triton.autotune(configs=runtime.get_tuned_config('min'), key=['M', 'N'])
@triton.jit
def min_kernel(inp, out_value, out_index, M, N, K, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr):
