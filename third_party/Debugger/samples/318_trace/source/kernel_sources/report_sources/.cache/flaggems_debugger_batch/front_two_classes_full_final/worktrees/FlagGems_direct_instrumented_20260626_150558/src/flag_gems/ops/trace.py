...
@libentry()
@triton.jit
def trace_kernel(inp_ptr, out_ptr, num_diag, stride0, stride1, BLOCK_SIZE: tl.constexpr):
...
def trace(self):
    ...
    if self.ndim != 2:
        raise RuntimeError(f'trace: expected a 2D tensor, but got a {self.ndim}D tensor')
    M, N = self.shape
    ...
    with torch_device_fn.device(self.device):
        trace_kernel[grid](self, out, num_diag, stride0, stride1, BLOCK_SIZE=BLOCK_SIZE)
    return out
