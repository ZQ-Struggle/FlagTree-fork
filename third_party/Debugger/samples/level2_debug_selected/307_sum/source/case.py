...
triton.enable_debug(
    level=int(os.environ.get("FLAGTREE_DEBUGGER_BATCH_LEVEL", "2")),
    addr_level=int(os.environ.get("FLAGTREE_DEBUGGER_BATCH_ADDR_LEVEL", "2")),
)
...
with manager:
    x = torch.linspace(-8, 7, 16, dtype=torch.float32, device=device).reshape(4, 4)
    result = torch.sum(x)
