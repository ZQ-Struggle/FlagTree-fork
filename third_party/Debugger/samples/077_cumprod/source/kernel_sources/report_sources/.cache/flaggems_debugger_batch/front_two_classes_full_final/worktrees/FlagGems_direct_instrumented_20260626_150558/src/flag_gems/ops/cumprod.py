...
def scan_part_product_kernel(inp, out, partial_product, n_elements, part_num, BLOCK_SIZE: tl.constexpr):
...
@libentry()
@triton.jit(do_not_specialize=['n_elements', 'part_num'])
def multiply_base_product_kernel(out, partial_product, n_elements, part_num, BLOCK_SIZE: tl.constexpr):
    pid = ext.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offset = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offset < n_elements
...
def scan_part_product_abc_kernel(inp, out, partial_product, B, C, part_num, BLOCK_SIZE: tl.constexpr):
...
@libentry()
@triton.jit(do_not_specialize=['part_num'])
def multiply_base_product_abc_kernel(out, partial_product, B, C, part_num, BLOCK_SIZE: tl.constexpr):
    pid_a = ext.program_id(0)
    pid_b = ext.program_id(1)
    pid_c = ext.program_id(2)
    a_idx = pid_a
...
def scan_then_fan_col(inp, out, n_ele, dtype):
    ...
    with torch_device_fn.device(inp.device):
        scan_part_product_kernel[grid](inp, out, partial_product, n_ele, part_num, BLOCK_SIZE)
    if part_num >= 2:
        partial_prefix = torch.empty_like(partial_product)
        scan_then_fan_col(partial_product, partial_prefix, part_num, dtype)
        with torch_device_fn.device(inp.device):
            multiply_base_product_kernel[grid](out, partial_prefix, n_ele, part_num, BLOCK_SIZE)
...
def scan_then_fan(inp, out, A, B, C, dtype):
    ...
    grid = (A, part_num, C)
    with torch_device_fn.device(inp.device):
        scan_part_product_abc_kernel[grid](inp, out, partial_product, B, C, part_num, BLOCK_SIZE)
    if part_num >= 2:
        partial_prefix = torch.empty_like(partial_product)
        scan_then_fan(partial_product, partial_prefix, A, part_num, C, dtype)
        with torch_device_fn.device(inp.device):
            multiply_base_product_abc_kernel[grid](out, partial_prefix, B, C, part_num, BLOCK_SIZE)
...
def _should_redispatch_on_ascend(dtype):
    return runtime_device.vendor_name == 'ascend' and (is_integer_dtype(dtype) or is_boolean_dtype(dtype))

def _scan_block_size(length):
    limit = ASCEND_SCAN_LIMIT if runtime_device.vendor_name == 'ascend' else CUDA_SMALL_SCAN_LIMIT
    if length <= limit:
...
def reduce_then_scan_row(x, out, M, N, compute_dtype):
    persistent_limit = ASCEND_SCAN_LIMIT if runtime_device.vendor_name == 'ascend' else 16384
    ...
    reduce_then_scan_block_product_kernel_row[M, num_ctas, 1, 1](x, block_products, N, tiles_per_cta, TILE_SIZE, num_warps=num_warps)
    reduce_then_scan_root_scan_kernel_row[M, 1, 1](block_products, block_inclusive_prefix, num_ctas, ROOT_SCAN_TILE_SIZE, num_warps=num_warps)
...
@triton.jit
def reduce_then_scan_block_product_kernel_row(in_ptr, block_product_ptr, N, tiles_per_cta, TILE_SIZE: tl.constexpr):
    ...
    for start in range(block_offset, block_end, TILE_SIZE):
        ...
        x = tl.load(in_ptr + pid_m * N + offsets, mask=offsets < N, other=1).to(acc_dtype)
        acc *= x
    block_product = tl.reduce(acc, axis=0, combine_fn=reduce_mul)
    tl.store(block_product_ptr + pid_m * num_programs_n + pid_n, block_product, cache_modifier='.cg')
    tl.debug_collect_end()
...
@triton.jit
def reduce_then_scan_block_scan_kernel_row(in_ptr, previous_product_ptr, out_ptr, N, num_tiles_n, tiles_per_cta, TILE_SIZE: tl.constexpr):
    pid_m = tl.program_id(0).to(tl.int64)
    pid_n = tl.program_id(1).to(tl.int64)
    block_offset = pid_n * (tiles_per_cta * TILE_SIZE)
    block_end = min(block_offset + tiles_per_cta * TILE_SIZE, N)
    acc_dtype: tl.constexpr = get_prod_accum_type(out_ptr.type.element_ty)
    tl.debug_collect_start(level=1, addr_level=1)
    prefix = tl.load(previous_product_ptr + pid_m * num_tiles_n + pid_n - 1, mask=pid_n > 0, other=1).to(acc_dtype)
    for start in range(block_offset, block_end, TILE_SIZE):
        offsets = start + tl.arange(0, TILE_SIZE)
        mask = offsets < N
...
def cumprod(inp, dim, *, dtype=None):
    ...
    if is_boolean_dtype(inp.dtype):
        if is_boolean_dtype(out_dtype):
            return torch.ops.aten.cumprod.default.redispatch(_FALLBACK_KEYSET, inp, dim, dtype=dtype)
        uint8_inp = inp.to(torch.uint8)
        if runtime_device.vendor_name == 'ascend':
            return torch.ops.aten.cumprod.default.redispatch(_FALLBACK_KEYSET, uint8_inp, dim, dtype=dtype)
...
def cumprod_(inp, dim, *, dtype=None):
    ...
    if dtype is not None and dtype != inp.dtype:
        raise RuntimeError('Bad in-place call: input tensor dtype and output tensor dtype should match')
    if is_boolean_dtype(inp.dtype):
        raise NotImplementedError('In-place cumprod is not supported for boolean tensors')
    if _should_redispatch_on_ascend(inp.dtype):
        return torch.ops.aten.cumprod_.default.redispatch(_FALLBACK_KEYSET, inp, dim, dtype=dtype)
