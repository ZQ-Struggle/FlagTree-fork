...
@libentry()
@libtuner(configs=runtime.get_tuned_config('mv'), key=['M', 'N'], strategy=['align32', 'align32'], flagtune_op_name='mv')
@triton.jit
...
def mv(inp, vec):
    ...
    with torch_device_fn.device(inp.device):
        mv_kernel[grid](inp, vec, out, N, M, inp.stride(0), inp.stride(1), vec.stride(0), out.stride(0))
    return out
