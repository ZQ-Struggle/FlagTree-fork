...
@libentry()
@triton.autotune(configs=[triton.Config({'tile_size': 1024}, num_stages=3, num_warps=1), triton.Config({'tile_size': 2048}, num_stages=3, num_warps=1), triton.Config({'tile_size': 4096}, num_stages=3, num_warps=1), triton.Config({'tile_size': 8192}, num_stages=3, num_warps=1), triton.Config({'tile_size': 16384}, num_stages=3, num_warps=1), triton.Config({'tile_size': 21760}, num_stages=3, num_warps=1), triton.Config({'tile_size': 32768}, num_stages=3, num_warps=1)], key=['M', 'N'], prune_configs_by={'early_config_prune': early_config_prune})
@triton.jit
...
def outer_(lhs, rhs):
    ...
    grid = lambda META: (min(triton.cdiv(m, META['BLOCK_M']) * triton.cdiv(n, META['BLOCK_N']), 65535),)
