...
_SUPPORTED_DATA_DTYPES = (torch.float16, torch.bfloat16, torch.float32, torch.float64)
...
def _all_lengths_equal(lengths, value):
    cache_key = (lengths.device.type, lengths.data_ptr(), tuple(lengths.shape), getattr(lengths, '_version', None), value)
...
def _wrap_axis(axis, ndim):
    if ndim == 0:
        raise IndexError('segment_reduce(): input tensor must have at least one dimension.')
    if axis < -ndim or axis >= ndim:
        raise IndexError(f'segment_reduce(): axis {axis} is out of bounds for tensor of dimension {ndim}.')
    return axis % ndim

def _check_reduce_and_dtype(data, reduce):
    if reduce not in _SUPPORTED_REDUCES:
        raise RuntimeError("segment_reduce(): reduce must be one of 'sum', 'mean', 'max', 'min', or 'prod'.")
...
def _check_index_tensor(data, index_tensor, name, axis):
    ...
    if index_tensor.device != data.device:
        raise RuntimeError(f'segment_reduce(): Expected data and {name} on the same device.')
    if data.dim() < index_tensor.dim():
        raise RuntimeError(f'segment_reduce(): Expected data.dim() >= {name}.dim(), got {data.dim()} and {index_tensor.dim()}.')
    if axis != index_tensor.dim() - 1:
        raise RuntimeError(f'segment_reduce(): Expected axis to be the last dimension of {name} but got {axis}.')
...
def _validate_lengths(data, lengths, axis, unsafe):
    ...
    if not valid_lengths:
        raise RuntimeError('segment_reduce(): Expected all rows of lengths along axis to sum to data.size(lengths.dim()-1) when !unsafe.')
...
@libentry()
@triton.jit
def _segment_reduce_uniform_other_backward_kernel(grad, output, data, grad_input, total_rows, segment_count, segment_length, inner_size, data_size_axis, IS_MAX: tl.constexpr, IS_MIN: tl.constexpr, IS_PROD: tl.constexpr, INITIAL_PROD_VALUE: tl.constexpr, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, BLOCK_K: tl.constexpr):
    ...
    data_offsets = outer_idx * data_size_axis * inner_size + (dim_idx * segment_length + seg_offsets) * inner_size + k_offsets
    ...
    grad_value = tl.load(grad + output_offsets, mask=output_mask, other=0.0).to(compute_dtype)
    output_value = tl.load(output + output_offsets, mask=output_mask, other=0.0).to(compute_dtype)
    if IS_MAX or IS_MIN:
        match = ((values != values) | (values == output_value[:, None, :])) & mask
        counter = tl.sum(match.to(tl.int64), axis=1)
        store_value = tl.where((counter >= 2) & (grad_value > 0), grad_value / counter, grad_value)
        ...
        zero_exclusive = tl.where(nan_count > 0, nan_scalar, tl.where(zero_count > 1, zero_scalar, product))
        nan_exclusive = tl.where(nan_count > 1, nan_scalar, tl.where(zero_count > 0, zero_scalar, product))
        exclusive = tl.where(nan_mask, nan_exclusive[:, None, :], zero_exclusive[:, None, :])
        grad_result = tl.where(nan_mask | zero_mask, grad_value[:, None, :] * exclusive, normal_grad)
        tl.store(grad_input + data_offsets, grad_result, mask=mask)
    tl.debug_collect_end()

@libentry()
@triton.jit
def _segment_reduce_uniform_sum_mean_backward_kernel(grad, grad_input, total_numel, segment_count, segment_length, inner_size, data_size_axis, IS_MEAN: tl.constexpr, BLOCK_SIZE: tl.constexpr):
...
@libentry()
@triton.jit
def _segment_reduce_uniform_inner1_forward_kernel(data, output, total_rows, segment_count, segment_length, data_size_axis, IS_SUM: tl.constexpr, IS_MEAN: tl.constexpr, IS_MAX: tl.constexpr, IS_MIN: tl.constexpr, IS_PROD: tl.constexpr, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr):
    pid = tle.program_id(0)
    data_dtype = data.dtype.element_ty
    compute_dtype = tl.float64 if data_dtype is tl.float64 else tl.float32
    tl.debug_collect_start(level=1, addr_level=1)
    ...
    if IS_SUM or IS_MEAN:
        ...
        values = tl.load(data + data_offsets, mask=mask, other=float('-inf')).to(compute_dtype)
        nan_mask = (values != values) & mask
...
@libentry()
@triton.jit
def _segment_reduce_uniform_forward_kernel(data, output, total_rows, segment_count, segment_length, inner_size, data_size_axis, IS_SUM: tl.constexpr, IS_MEAN: tl.constexpr, IS_MAX: tl.constexpr, IS_MIN: tl.constexpr, IS_PROD: tl.constexpr, BLOCK_M: tl.constexpr, BLOCK_K: tl.constexpr):
    pid_m = tle.program_id(0)
    pid_k = tle.program_id(1)
    data_dtype = data.dtype.element_ty
    compute_dtype = tl.float64 if data_dtype is tl.float64 else tl.float32
    ...
    base_offsets = outer_idx * data_size_axis * inner_size + segment_start * inner_size + k_offsets
    if IS_MAX:
    ...
    if segment_length <= _UNIFORM_KERNEL_MAX_SEGMENT_LENGTH:
        ...
        if inner_size == 1:
            ...
            with torch_device_fn.device(data.device):
                _segment_reduce_uniform_inner1_forward_kernel[grid](data, output, total_rows, segment_count, segment_length, data.shape[axis], reduce == 'sum', reduce == 'mean', reduce == 'max', reduce == 'min', reduce == 'prod', BLOCK_M=block_m, BLOCK_N=block_n)
            return output
        ...
        with torch_device_fn.device(data.device):
            _segment_reduce_uniform_forward_kernel[grid](data, output, total_rows, segment_count, segment_length, inner_size, data.shape[axis], reduce == 'sum', reduce == 'mean', reduce == 'max', reduce == 'min', reduce == 'prod', BLOCK_M=block_m, BLOCK_K=block_k)
        return output
    if data.device.type == 'npu':
        return None
    view_shape = data.shape[:axis] + (segment_count, segment_length) + data.shape[axis + 1:]
...
def _segment_reduce_uniform_sum_mean_backward(data, grad, reduce, lengths, axis):
    ...
    with torch_device_fn.device(data.device):
        _segment_reduce_uniform_sum_mean_backward_kernel[grid](grad, grad_input, data.numel(), segment_count, segment_length, inner_size, data.shape[axis], reduce == 'mean', BLOCK_SIZE=block_size)
    return grad_input

def _segment_reduce_uniform_other_backward(data, output, grad, reduce, lengths, axis, initial):
    segment_count = lengths.shape[-1]
    segment_length = _get_uniform_segment_length(data, lengths, axis)
    ...
    inner_size = _prod(data.shape[axis + 1:])
    total_rows = _prod(lengths.shape)
    block_m, block_k = _get_uniform_backward_tile_config(data.device, inner_size, reduce, data.dtype)
    block_n = min(_get_block_size(data.device), triton.next_power_of_2(segment_length))
    _, initial_prod_value = _make_initial('prod', initial)
    grid = (triton.cdiv(total_rows, block_m), triton.cdiv(inner_size, block_k))
    with torch_device_fn.device(data.device):
        _segment_reduce_uniform_other_backward_kernel[grid](grad, output, data, grad_input, total_rows, segment_count, segment_length, inner_size, data.shape[axis], reduce == 'max', reduce == 'min', reduce == 'prod', initial_prod_value, BLOCK_M=block_m, BLOCK_N=block_n, BLOCK_K=block_k)
    return grad_input

@libentry()
@triton.jit
def _lengths_to_offsets_kernel(lengths, offsets, outer_count, segment_count):
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
...
@libentry()
@triton.jit
def _segment_reduce_forward_kernel(data, offsets, output, segment_count, inner_size, data_size_axis, IS_SUM: tl.constexpr, IS_MEAN: tl.constexpr, IS_MAX: tl.constexpr, IS_MIN: tl.constexpr, IS_PROD: tl.constexpr, HAS_INITIAL: tl.constexpr, INITIAL_VALUE: tl.constexpr, BLOCK_SIZE: tl.constexpr):
    pid = tle.program_id(0)
    data_dtype = data.dtype.element_ty
    compute_dtype = tl.float64 if data_dtype is tl.float64 else tl.float32
    ...
    if IS_PROD:
        ...
        while pos < segment_end:
            data_offset = outer_idx * data_size_axis * inner_size + pos * inner_size + inner_idx
        ...
        while pos < segment_end:
            ...
            data_offsets = outer_idx * data_size_axis * inner_size + segment_offsets * inner_size + inner_idx
            if IS_SUM or IS_MEAN:
                values = tl.load(data + data_offsets, mask=mask, other=0.0).to(compute_dtype)
                ...
                chunk = tl.max(tl.where(mask & ~nan_mask, values, float('-inf')), axis=0)
...
@libentry()
@triton.jit
def _segment_reduce_backward_kernel(grad, output, data, offsets, grad_input, segment_count, inner_size, data_size_axis, IS_SUM: tl.constexpr, IS_MEAN: tl.constexpr, IS_MAX: tl.constexpr, IS_MIN: tl.constexpr, IS_PROD: tl.constexpr, INITIAL_PROD_VALUE: tl.constexpr, BLOCK_SIZE: tl.constexpr):
    pid = tle.program_id(0)
    data_dtype = data.dtype.element_ty
    compute_dtype = tl.float64 if data_dtype is tl.float64 else tl.float32
    inner_idx = pid % inner_size
    ...
    if segment_length > 0:
        ...
        if IS_SUM or IS_MEAN:
            ...
            while pos < segment_end:
                ...
                grad_result = tl.where(nan_mask | zero_mask, grad_value * exclusive, normal_grad)
                tl.store(grad_input + data_offsets, grad_result, mask=mask)
                pos += BLOCK_SIZE
    tl.debug_collect_end()
...
def _lengths_to_offsets(lengths):
    ...
    if offsets.numel() > 0:
        with torch_device_fn.device(lengths.device):
            _lengths_to_offsets_kernel[outer_count,](lengths, offsets, outer_count, segment_count)
    return offsets
...
def _prepare_common(data, reduce, lengths, offsets, indices, axis, unsafe):
    ...
    if indices is not None:
        raise RuntimeError('segment_reduce(): indices based reduction is not supported yet.')
    if offsets is not None:
        _check_index_tensor(data, offsets, 'offsets', axis)
        offsets_contig = offsets.contiguous()
        segment_count = offsets_contig.shape[-1] - 1
        output_shape = offsets_contig.shape[:-1] + (segment_count,) + data.shape[axis + 1:]
...
def segment_reduce(data, reduce, *, lengths=None, indices=None, offsets=None, axis=0, unsafe=False, initial=None):
    ...
    if initial is None and lengths is not None and (offsets is None):
        ...
        uniform_result = _segment_reduce_uniform_lengths(data_contig, reduce, lengths, axis)
        if uniform_result is not None:
            return uniform_result
    axis, offsets_contig, output_shape, _ = _prepare_common(data, reduce, lengths, offsets, indices, axis, unsafe)
    data_contig = data.contiguous()
    output = torch.empty(output_shape, dtype=data.dtype, device=data.device)
    if output.numel() == 0:
        return output
    ...
    grid = (output.numel(),)
    with torch_device_fn.device(data.device):
        _segment_reduce_forward_kernel[grid](data_contig, offsets_contig, output, segment_count, inner_size, data_size_axis, reduce == 'sum', reduce == 'mean', reduce == 'max', reduce == 'min', reduce == 'prod', has_initial, initial_value, BLOCK_SIZE=_get_block_size(data.device))
    return output

def segment_reduce_out(data, reduce, *, lengths=None, indices=None, offsets=None, axis=0, unsafe=False, initial=None, out):
    logger.debug('GEMS SEGMENT_REDUCE_OUT')
    result = segment_reduce(data, reduce, lengths=lengths, indices=indices, offsets=offsets, axis=axis, unsafe=unsafe, initial=initial)
...
def _segment_reduce_backward(grad, output, data, reduce, *, lengths=None, offsets=None, axis=0, initial=None):
    ...
    if initial is None and lengths is not None and (offsets is None) and (reduce in _SUPPORTED_REDUCES):
        _check_reduce_and_dtype(data, reduce)
        axis = _wrap_axis(axis, data.dim())
        _check_index_tensor(data, lengths, 'lengths', axis)
        if _is_unit_lengths(data, lengths, axis):
    ...
    if lengths is not None and offsets is None and (reduce in ('sum', 'mean')):
        ...
        uniform_result = _segment_reduce_uniform_sum_mean_backward(data_contig, grad_contig, reduce, lengths, axis)
        if uniform_result is not None:
            return uniform_result
    if lengths is not None and offsets is None and (reduce in ('max', 'min', 'prod')):
        ...
        uniform_result = _segment_reduce_uniform_other_backward(data_contig, output_contig, grad_contig, reduce, lengths, axis, initial)
        if uniform_result is not None:
            return uniform_result
    axis, offsets_contig, output_shape, _ = _prepare_common(data, reduce, lengths, offsets, None, axis, True)
    data_contig = data.contiguous()
    grad_contig = grad.contiguous()
    output_contig = output.contiguous()
    ...
    with torch_device_fn.device(data.device):
        _segment_reduce_backward_kernel[grid](grad_contig, output_contig, data_contig, offsets_contig, grad_input, segment_count, inner_size, data_size_axis, reduce == 'sum', reduce == 'mean', reduce == 'max', reduce == 'min', reduce == 'prod', initial_prod_value, BLOCK_SIZE=_get_block_size(data.device))
    return grad_input

def _segment_reduce_backward_out(grad, output, data, reduce, *, lengths=None, offsets=None, axis=0, initial=None, out):
    logger.debug('GEMS _SEGMENT_REDUCE_BACKWARD_OUT')
    result = _segment_reduce_backward(grad, output, data, reduce, lengths=lengths, offsets=offsets, axis=axis, initial=initial)
    if out.shape != result.shape:
        out.resize_(result.shape)
    out.copy_(result)
    return out
