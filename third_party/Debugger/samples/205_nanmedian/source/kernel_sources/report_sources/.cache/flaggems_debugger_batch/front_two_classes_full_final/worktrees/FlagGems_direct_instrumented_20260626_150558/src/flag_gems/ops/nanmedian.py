...
FLAT_RADIX_BITS = 8
RADIX_SELECT_DTYPES = (torch.float16, torch.bfloat16, torch.float32, torch.int8, torch.uint8, torch.int16, torch.int32)
ASCEND_HISTOGRAM_SELECT_DTYPES = (torch.int8, torch.uint8)
ASCEND_BYTE_HISTOGRAM_SELECT_DTYPES = (torch.int16, torch.int32)
ASCEND_FLOAT_SELECT_DTYPES = (torch.float16, torch.float32)
ASCEND_HISTOGRAM_BINS = 256
ASCEND_MULTI_HISTOGRAM_MIN_N = 8192
ASCEND_FLAT_SORT_DTYPES = (torch.float16, torch.float32, torch.int8, torch.uint8, torch.int16, torch.int32)
...
@libentry()
@triton.jit
def count_valid_kernel(inp, valid_counts, M, N: tl.constexpr, BLOCK_N: tl.constexpr, USE_ISNAN: tl.constexpr):
...
@libentry()
@triton.jit
def nanmedian_select_kernel(inp, out_values, out_indices, M, N: tl.constexpr, BLOCK_N: tl.constexpr, USE_ISNAN: tl.constexpr):
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = tl.arange(0, BLOCK_N)
    mask = offsets < N
...
@libentry()
@triton.jit
def nanmedian_float_clean_count_kernel(inp, cleaned, valid_counts, N: tl.constexpr, BLOCK_N: tl.constexpr):
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = tl.arange(0, BLOCK_N)
    dtype = inp.dtype.element_ty
...
@libentry()
@triton.jit
def nanmedian_float_sorted_gather_kernel(sorted_values, sorted_indices, valid_counts, out_values, out_indices, N: tl.constexpr):
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    count = tl.load(valid_counts + pid)
    rank = tl.where(count > 0, (count - 1) // 2, 0)
    result_val = tl.load(sorted_values + pid * N + rank, mask=count > 0, other=float('nan'))
...
@libentry()
@triton.jit
def nanmedian_ascend_histogram_select_kernel(inp, out_values, out_indices, M, N: tl.constexpr, BLOCK_N: tl.constexpr, HISTOGRAM_BINS: tl.constexpr):
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = tl.arange(0, BLOCK_N)
...
@libentry()
@triton.jit
def nanmedian_ascend_histogram_count_kernel(inp, partial_counts, M, N: tl.constexpr, BLOCK_N: tl.constexpr, NUM_CHUNKS: tl.constexpr, HISTOGRAM_BINS: tl.constexpr):
...
@libentry()
@triton.jit
def nanmedian_ascend_histogram_reduce_kernel(inp, partial_counts, out_values, out_indices, M, N: tl.constexpr, BLOCK_N: tl.constexpr, NUM_CHUNKS: tl.constexpr, HISTOGRAM_BINS: tl.constexpr):
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = tl.arange(0, BLOCK_N)
    bins = tl.arange(0, HISTOGRAM_BINS)
...
@libentry()
@triton.jit
def nanmedian_ascend_byte_histogram_select_kernel(inp, out_values, out_indices, M, N: tl.constexpr, BLOCK_N: tl.constexpr, HISTOGRAM_BINS: tl.constexpr):
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = tl.arange(0, BLOCK_N)
    bins = tl.arange(0, HISTOGRAM_BINS)
...
@libentry()
@triton.jit
def nanmedian_ascend_byte_histogram_init_kernel(state, M, N: tl.constexpr):
    pid = tle.program_id(0)
    base = pid * 3
    tl.debug_collect_start(level=1, addr_level=1)
...
@libentry()
@triton.jit
def nanmedian_ascend_byte_histogram_count_kernel(inp, state, partial_counts, M, N: tl.constexpr, BLOCK_N: tl.constexpr, NUM_CHUNKS: tl.constexpr, HISTOGRAM_BINS: tl.constexpr, DIGIT_POS: tl.constexpr):
    pid_m = tle.program_id(0)
    pid_chunk = tle.program_id(1)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = pid_chunk * BLOCK_N + tl.arange(0, BLOCK_N)
...
@libentry()
@triton.jit
def nanmedian_ascend_byte_histogram_update_kernel(inp, partial_counts, state, M, NUM_CHUNKS: tl.constexpr, HISTOGRAM_BINS: tl.constexpr, DIGIT_POS: tl.constexpr):
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    bins = tl.arange(0, HISTOGRAM_BINS)
    counts = tl.zeros((HISTOGRAM_BINS,), dtype=tl.int32)
...
@libentry()
@triton.jit
def nanmedian_ascend_byte_histogram_find_index_kernel(inp, state, out_values, out_indices, M, N: tl.constexpr, BLOCK_N: tl.constexpr):
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = tl.arange(0, BLOCK_N)
    dtype = inp.dtype.element_ty
...
@libentry()
@triton.jit
def nanmedian_radix_select_kernel(inp, out_values, out_indices, M, N: tl.constexpr, BLOCK_N: tl.constexpr, RADIX_BITS_: tl.constexpr, USE_ISNAN: tl.constexpr, USE_HISTOGRAM: tl.constexpr):
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = tl.arange(0, BLOCK_N)
    dtype = inp.dtype.element_ty
    ...
    if dtype.is_floating():
        ...
        fallback_value = get_dtype_min(dtype)
    result_val = tl.load(inp + pid * N + result_idx, mask=valid_count > 0, other=fallback_value)
    if dtype.is_floating():
        all_nan = valid_count == 0
...
@libentry()
@triton.jit
def flat_radix_init_kernel(valid_count, state, result_idx, N: tl.constexpr, IS_FLOAT: tl.constexpr):
    tl.debug_collect_start(level=1, addr_level=1)
...
@libentry()
@triton.jit
def flat_radix_count_valid_kernel(inp, valid_count, N: tl.constexpr, BLOCK_N: tl.constexpr, USE_ISNAN: tl.constexpr):
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
...
@libentry()
@triton.jit
def flat_radix_count_kernel(inp, bin_counts, state, N: tl.constexpr, BLOCK_N: tl.constexpr, DIGIT_POS: tl.constexpr, RADIX_BITS_: tl.constexpr, RADIX_SIZE: tl.constexpr, USE_ISNAN: tl.constexpr, USE_HISTOGRAM: tl.constexpr):
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = pid * BLOCK_N + tl.arange(0, BLOCK_N)
    mask = offsets < N
...
@libentry()
@triton.jit
def flat_radix_update_kernel(bin_counts, state, DIGIT_POS: tl.constexpr, RADIX_BITS_: tl.constexpr, RADIX_SIZE: tl.constexpr):
    tl.debug_collect_start(level=1, addr_level=1)
    bins = tl.arange(0, RADIX_SIZE)
...
@libentry()
@triton.jit
def flat_radix_find_index_kernel(inp, state, valid_count, result_idx, N: tl.constexpr, BLOCK_N: tl.constexpr, USE_ISNAN: tl.constexpr):
    tl.debug_collect_start(level=1, addr_level=1)
    ...
    if torch.is_floating_point(inp):
        ...
        if max_count - min_count <= 1:
            ...
            if min_k == max_k:
                ...
                return NanMedian(values=torch.where(positive, values, fallback.values), indices=torch.where(positive, indices, fallback.indices))
            result = _full_nan_result((M,), inp.dtype, inp.device)
            if min_count > 0:
                values, indices = torch.kthvalue(inp, min_k, dim=1)
                mask = valid_count == min_count
                result = NanMedian(values=torch.where(mask, values, result.values), indices=torch.where(mask, indices, result.indices))
            values, indices = torch.kthvalue(inp, max_k, dim=1)
            mask = valid_count == max_count
            return NanMedian(values=torch.where(mask, values, result.values), indices=torch.where(mask, indices, result.indices))
        result = _full_nan_result((M,), inp.dtype, inp.device)
...
def _nanmedian_ascend_float_sort_select(inp, M, N, values, indices):
    ...
    if N <= LARGE_FLOAT_REDUCTION_N:
        ...
        with torch_device_fn.device(inp.device):
            nanmedian_float_clean_count_kernel[M,](inp, cleaned, valid_counts, N, block_n, num_warps=num_warps, num_stages=1)
        sorted_values, sorted_indices = torch.sort(cleaned, dim=1)
    else:
        sorted_values, sorted_indices = torch.sort(inp, dim=1)
        valid_counts = torch.sum((sorted_values == sorted_values).to(torch.int32), dim=1)
    with torch_device_fn.device(inp.device):
        nanmedian_float_sorted_gather_kernel[M,](sorted_values, sorted_indices, valid_counts, flat_values, flat_indices, N, num_warps=1, num_stages=1)
...
def _nanmedian_dim_impl(inp, dim, keepdim, out=None, use_ascend_float_select=True):
    ...
    if N == 0:
        if M != 0:
            raise IndexError(f'median(): Expected reduction dim {dim} to have non-zero size.')
        if out is None:
    ...
    use_cuda_histogram = is_cuda and CUDA_SUPPORTS_MASKED_HISTOGRAM and (N > MAX_BLOCK_N) and (N == triton.next_power_of_2(N))
    use_ascend_float_select_path = use_ascend_float_select and is_ascend and (inp.dtype in ASCEND_FLOAT_SELECT_DTYPES) and in_radix_range
    use_ascend_histogram = is_ascend and inp.dtype in ASCEND_HISTOGRAM_SELECT_DTYPES and in_radix_range
    use_ascend_byte_histogram = is_ascend and inp.dtype in ASCEND_BYTE_HISTOGRAM_SELECT_DTYPES and in_radix_range
    if is_cuda and inp.dtype in RADIX_SELECT_DTYPES and in_radix_range:
        ...
        num_warps = 4 if block_n <= 512 else 8
        with torch_device_fn.device(inp.device):
            nanmedian_radix_select_kernel[M,](inp, flat_values, flat_indices, M, N, block_n, _radix_bits(inp, N) if use_cuda_histogram else RADIX_BITS, is_cuda, use_cuda_histogram, num_warps=num_warps, num_stages=1)
        ...
        partial_counts = torch.empty((M, num_chunks, ASCEND_HISTOGRAM_BINS), dtype=torch.int32, device=inp.device)
        num_warps = 4 if block_n <= 512 else 8
        with torch_device_fn.device(inp.device):
            nanmedian_ascend_histogram_count_kernel[M, num_chunks](inp, partial_counts, M, N, block_n, num_chunks, ASCEND_HISTOGRAM_BINS, num_warps=num_warps, num_stages=1)
            nanmedian_ascend_histogram_reduce_kernel[M,](inp, partial_counts, flat_values, flat_indices, M, N, block_n, num_chunks, ASCEND_HISTOGRAM_BINS, num_warps=num_warps, num_stages=1)
        ...
        num_warps = 4 if block_n <= 512 else 8
        with torch_device_fn.device(inp.device):
            nanmedian_ascend_histogram_select_kernel[M,](inp, flat_values, flat_indices, M, N, block_n, ASCEND_HISTOGRAM_BINS, num_warps=num_warps, num_stages=1)
        ...
        with torch_device_fn.device(inp.device):
            nanmedian_ascend_byte_histogram_init_kernel[M,](state, M, N, num_warps=1, num_stages=1)
            for digit_pos in range(nbits - 8, -1, -8):
                nanmedian_ascend_byte_histogram_count_kernel[M, num_chunks](inp, state, partial_counts, M, N, block_n, num_chunks, ASCEND_HISTOGRAM_BINS, digit_pos, num_warps=num_warps, num_stages=1)
                nanmedian_ascend_byte_histogram_update_kernel[M,](inp, partial_counts, state, M, num_chunks, ASCEND_HISTOGRAM_BINS, digit_pos, num_warps=num_warps, num_stages=1)
            nanmedian_ascend_byte_histogram_find_index_kernel[M,](inp, state, flat_values, flat_indices, M, N, block_n, num_warps=num_warps, num_stages=1)
        ...
        num_warps = 4 if block_n <= 512 else 8
        with torch_device_fn.device(inp.device):
            nanmedian_ascend_byte_histogram_select_kernel[M,](inp, flat_values, flat_indices, M, N, block_n, ASCEND_HISTOGRAM_BINS, num_warps=num_warps, num_stages=1)
    elif N <= MAX_BLOCK_N and inp.dtype is not torch.float64:
        flat_values = values.reshape(M)
        flat_indices = indices.reshape(M)
        block_n = triton.next_power_of_2(N)
        with torch_device_fn.device(inp.device):
            nanmedian_select_kernel[M,](inp, flat_values, flat_indices, M, N, block_n, is_cuda)
    else:
...
def _nanmedian_cuda_flat_radix_select(inp, out=None):
    ...
    with torch_device_fn.device(flat.device):
        flat_radix_init_kernel[1,](valid_count, state, result_idx, n, torch.is_floating_point(flat))
        if torch.is_floating_point(flat):
            flat_radix_count_valid_kernel[grid](flat, valid_count, n, block_n, True, num_warps=8, num_stages=1)
        flat_radix_init_rank_kernel[1,](valid_count, state)
        for digit_pos in range(nbits - radix_bits, -1, -radix_bits):
            bin_counts.zero_()
            flat_radix_count_kernel[grid](flat, bin_counts, state, n, block_n, digit_pos, radix_bits, radix_size, True, use_histogram, num_warps=8, num_stages=1)
            flat_radix_update_kernel[1,](bin_counts, state, digit_pos, radix_bits, radix_size, num_warps=8, num_stages=1)
        flat_radix_find_index_kernel[grid](flat, state, valid_count, result_idx, n, block_n, True, num_warps=8, num_stages=1)
...
def _nanmedian_flat_impl(inp, out=None):
    ...
    if n == 0:
        ...
        if out is not None:
            ...
            return out
        return result
    if inp.is_cuda and inp.dtype in RADIX_SELECT_DTYPES and (LONG_RADIX_REDUCTION_N < n <= INT32_MAX):
        return _nanmedian_cuda_flat_radix_select(inp, out=out)
    if inp.device.type == 'npu' and inp.dtype in ASCEND_FLAT_SORT_DTYPES and (n >= ASCEND_FLAT_SORT_MIN_N):
    ...
    _nanmedian_dim_impl(flat, 0, False, out=(out, indices), use_ascend_float_select=False)
    return out
