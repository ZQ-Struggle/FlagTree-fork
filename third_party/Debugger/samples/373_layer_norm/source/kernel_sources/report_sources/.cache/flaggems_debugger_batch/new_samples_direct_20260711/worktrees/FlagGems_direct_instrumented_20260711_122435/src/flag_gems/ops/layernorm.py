...
@libentry()
@triton.autotune(configs=runtime.get_tuned_config('layer_norm_persistent'), key=['M', 'N'])
@triton.jit(do_not_specialize=['eps'])
def layer_norm_persistent_kernel(in_ptr, out_ptr, weight_ptr, bias_ptr, out_mean_ptr, out_rstd_ptr, M, N, eps, TILE_N: tl.constexpr):
...
@triton.autotune(configs=runtime.get_tuned_config('layer_norm_persistent'), key=['M', 'N'])
@triton.jit(do_not_specialize=['eps'])
def layer_norm_persistent_kernel_multiline(in_ptr, out_ptr, weight_ptr, bias_ptr, out_mean_ptr, out_rstd_ptr, M, N, eps, TILE_M: tl.constexpr, TILE_N: tl.constexpr):
    pid = ext.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
...
@libentry()
@triton.autotune(configs=runtime.get_tuned_config('layer_norm_loop'), key=['M', 'N'])
@triton.jit(do_not_specialize=['eps'])
def layer_norm_loop_kernel(in_ptr, out_ptr, weight_ptr, bias_ptr, out_mean_ptr, out_rstd_ptr, M, N, eps, TILE_N: tl.constexpr):
    pid = ext.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
...
@libentry()
@triton.autotune(configs=runtime.get_tuned_config('layer_norm_backward'), key=['M', 'N'])
@triton.jit
def layer_norm_backward_kernel(dY, X, W, Mean, Rstd, dX, M, N, BLOCK_ROW_SIZE: tl.constexpr, BLOCK_COL_SIZE: tl.constexpr):
    tl.debug_collect_start(level=1, addr_level=1)
...
@libentry()
@triton.autotune(configs=runtime.get_tuned_config('weight_bias_backward'), key=['N'])
@triton.jit
def weight_bias_backward_kernel(dY, X, Mean, Rstd, dW, dB, M, N, BLOCK_ROW_SIZE: tl.constexpr, BLOCK_COL_SIZE: tl.constexpr):
    tl.debug_collect_start(level=1, addr_level=1)
...
def layer_norm(input, normalized_shape, weight=None, bias=None, eps=1e-05):
    ...
    with torch_device_fn.device(input.device):
        if N <= 128:
            ...
            layer_norm_persistent_kernel_multiline[grid](input, y, weight, bias, mean, rstd, M, N, eps, TILE_M, TILE_N)
        elif N <= 4096:
            TILE_N = triton.next_power_of_2(N)
            grid = (M, 1, 1)
            layer_norm_persistent_kernel[grid](input, y, weight, bias, mean, rstd, M, N, eps, TILE_N)
        else:
            grid = (M, 1, 1)
            layer_norm_loop_kernel[grid](input, y, weight, bias, mean, rstd, M, N, eps)
    return (y, mean, rstd)

def layer_norm_backward(grad_out, input, normalized_shape, mean, rstd, weight=None, bias=None, output_mask=None):
    ...
    if output_mask[0]:
        ...
        with torch_device_fn.device(input.device):
            layer_norm_backward_kernel[grid](grad_out, input, weight, mean, rstd, in_grad, M, N)
    else:
        in_grad = None
    ...
    with torch_device_fn.device(input.device):
        weight_bias_backward_kernel[grid](grad_out, input, mean, rstd, weight_grad, bias_grad, M, N)
