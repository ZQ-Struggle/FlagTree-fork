...
@triton.jit
def slice_backward_kernel(grad_output_ptr, grad_input_ptr, numel, inner, slice_len, dim_size, start, step, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
...
def slice_backward(grad_output, input_sizes, dim, start, end, step):
    ...
    slice_backward_kernel[grid](grad_output, grad_input, numel, inner, slice_len, dim_size, start, step, BLOCK_SIZE=BLOCK)
    return grad_input
