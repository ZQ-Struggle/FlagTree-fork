...
@triton.jit
def _gather_flat_kernel_fixed(inp, index, out, base_offset, inp_dim_stride, N, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
...
def gather_flat_fixed(inp: torch.Tensor, dim: int, index: torch.Tensor, out=None):
    ...
    _gather_flat_kernel_fixed[grid](inp_strided, index, out, base_offset, dim_stride, N)
    return out

def gather(inp, dim, index, out=None, sparse_grad=False):
    logger.debug('GEMS_ASCEND GATHER')
    if inp.ndim != index.ndim:
        raise IndexError(f'self and index must have the same number of dimensions, got self.ndim = {inp.ndim} and index.ndim = {index.ndim}')
    if out is None:
        out = torch.empty_like(index, dtype=inp.dtype, device=inp.device)
    dim = dim % inp.dim()
