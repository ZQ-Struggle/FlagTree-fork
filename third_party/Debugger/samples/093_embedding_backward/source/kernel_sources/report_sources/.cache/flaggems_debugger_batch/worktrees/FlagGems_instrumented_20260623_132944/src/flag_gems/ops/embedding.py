...
@libentry()
@triton.jit
def embedding_kernel(out_ptr, in_ptr, weight_ptr, N: tl.constexpr, BLOCK_SIZE: tl.constexpr):
...
@libentry()
@triton.jit
def indice_freq_kernel(indices_freq, indices, elem_cnt: tl.constexpr, INDICE_BLOCK_SIZE: tl.constexpr):
    pid = ext.program_id(0)
...
def embedding_backward_kernel(grad_in, grad_out, indices, padding_idx, HAS_PADDING_IDX: tl.constexpr, N: tl.constexpr, BLOCK_SIZE: tl.constexpr):
    pid = ext.program_id(0)
    grad_out += pid * N
    indices += pid
    tl.debug_collect_start(level=1, addr_level=1)
...
def embedding_grad_scale_kernel(grad_out, indice_freq, n_rows, N, BLOCK_SIZE: tl.constexpr):
...
def embedding_backward(grad_outputs, indices, num_weights, padding_idx=-1, scale_grad_by_freq=False, sparse=False):
    logger.debug('GEMS EMBEDDING BACKWARD')
    assert not sparse, 'Currently do not support sparse format'
    M = indices.numel()
    N = grad_outputs.shape[-1]
    grad_inputs = torch.zeros((num_weights, grad_outputs.shape[-1]), device=grad_outputs.device, dtype=torch.float32 if grad_outputs.dtype is torch.bfloat16 else grad_outputs.dtype)
    if scale_grad_by_freq:
        indice_freq = torch.zeros((num_weights,), requires_grad=False, device=grad_outputs.device, dtype=torch.int32)
    ...
    with torch_device_fn.device(grad_outputs.device):
        embedding_backward_kernel[M,](grad_inputs, grad_outputs, indices, padding_idx, HAS_PADDING_IDX, N, BLOCK_SIZE)
    if scale_grad_by_freq:
        with torch_device_fn.device(grad_outputs.device):
            embedding_grad_scale_kernel[M,](grad_inputs, indice_freq, num_weights, N, BLOCK_SIZE)
    return grad_inputs.to(torch.bfloat16) if grad_outputs.dtype is torch.bfloat16 else grad_inputs
