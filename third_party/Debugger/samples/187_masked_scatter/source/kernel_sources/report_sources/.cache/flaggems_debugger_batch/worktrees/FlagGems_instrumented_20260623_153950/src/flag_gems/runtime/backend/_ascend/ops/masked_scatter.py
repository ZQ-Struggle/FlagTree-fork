...
@libentry()
@triton.jit
def masked_scatter_single_pass_kernel(inp_ptr, mask_ptr, src_ptr, N, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
...
def masked_scatter_kernel(inp_ptr, mask_ptr, src_ptr, part_sums_ptr, N, num_blocks, num_blocks_per_row, BLOCK_SIZE: tl.constexpr):
...
def masked_scatter_impl(inp, mask, source, N):
    ...
    if N <= 4096:
        ...
        masked_scatter_single_pass_kernel[1,](inp, mask, source, N, BLOCK_SIZE=BLOCK_SIZE)
        return inp
    ...
    with torch_device_fn.device(inp.device):
        ...
        count_mask_per_block_kernel[n_blocks,](mask, block_counts, N, BLOCK_SIZE=BLOCK_SIZE)
        ...
        masked_scatter_kernel[n_blocks,](inp, mask, source, part_sums, N, n_blocks, 1, BLOCK_SIZE=BLOCK_SIZE)
    return inp

def masked_scatter(inp, mask, source):
    logger.debug('GEMS_ASCEND MASKED SCATTER')
    assert broadcastable(inp.shape, mask.shape), 'The shapes of the `mask` and the `input` tensor must be broadcastable'
...
def masked_scatter_(inp, mask, source):
    ...
    if not inp.is_contiguous():
        raise RuntimeError('in-place operation currently requires contiguous input tensor. ')
