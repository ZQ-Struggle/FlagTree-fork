...
def tl_cummin(input, index, axis=0):
    return tl.associative_scan((input, index), axis, tle.minimum_with_index_tie_break_right)

@triton.jit
def tl_min_tie_break_right(input, index, axis=None, keep_dims=False):
    return tl.reduce((input, index), axis, tle.minimum_with_index_tie_break_right, keep_dims=keep_dims)

@libentry()
@triton.jit(do_not_specialize=['n_elements'])
def add_base_min_kernel(out, out_indices, partial_min, partial_min_indices, n_elements, BLOCK_SIZE: tl.constexpr):
...
@libentry()
@triton.jit(do_not_specialize=['n_elements'])
def scan_part_min_kernel(inp, out, in_indices, out_indices, partial_min, partial_min_indices, n_elements, BLOCK_SIZE: tl.constexpr, NEED_PARTIAL: tl.constexpr, USE_OUT_INDICES: tl.constexpr):
    pid = tle.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offset = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offset < n_elements
    ...
    if tl.constexpr(NEED_PARTIAL):
        part_min_via_min, part_min_indices_via_min = tl_min_tie_break_right(inp_vals, in_indices_vals, axis=0)
...
def scan_then_fan_col(inp, out, out_indices, n_ele, dtype, use_out_indices=False):
    ...
    with torch_device_fn.device(inp.device):
        scan_part_min_kernel[grid](inp, out, out_indices, out_indices, partial_min, partial_min_indices, n_ele, BLOCK_SIZE, need_partial, use_out_indices)
    if part_num >= 2:
        scan_then_fan_col(partial_min, partial_min, partial_min_indices, part_num, dtype, use_out_indices=True)
        with torch_device_fn.device(inp.device):
            add_base_min_kernel[grid](out, out_indices, partial_min, partial_min_indices, n_ele, BLOCK_SIZE)

@libentry()
@triton.jit(do_not_specialize=['part_num'])
def scan_part_min_abc_kernel(inp, out, in_indices, out_indices, partial_min, partial_min_indices, B, C, part_num, BLOCK_SIZE: tl.constexpr, NEED_PARTIAL: tl.constexpr, USE_OUT_INDICES: tl.constexpr):
...
@libentry()
@triton.jit(do_not_specialize=['part_num'])
def add_base_min_abc_kernel(out, out_indices, partial_min, partial_min_indices, B, C, part_num, BLOCK_SIZE: tl.constexpr):
    pid_a = tle.program_id(0)
    pid_b = tle.program_id(1)
    pid_c = tle.program_id(2)
    a_idx = pid_a
...
def scan_then_fan(inp, out, out_indices, A, B, C, dtype, use_out_indices=False):
    ...
    if need_partial:
        ...
        partial_min_indices = torch.empty(A, part_num, C, dtype=torch.int64, device=inp.device)
    else:
        partial_min = None
        partial_min_indices = None
    grid = (A, part_num, C)
    with torch_device_fn.device(inp.device):
        scan_part_min_abc_kernel[grid](inp, out, out_indices, out_indices, partial_min, partial_min_indices, B, C, part_num, BLOCK_SIZE, need_partial, use_out_indices)
    if part_num >= 2:
        scan_then_fan(partial_min, partial_min, partial_min_indices, A, part_num, C, dtype, use_out_indices=True)
        with torch_device_fn.device(inp.device):
            add_base_min_abc_kernel[grid](out, out_indices, partial_min, partial_min_indices, B, C, part_num, BLOCK_SIZE)

@libentry()
@triton.jit()
def scan_part_min_abc_loop_kernel(inp, out, out_indices, B, C, loop_num, BLOCK_SIZE: tl.constexpr):
    pid_a = tle.program_id(0)
    ...
    max_value = get_dtype_max(inp.type.element_ty)
    if tl.constexpr(inp.type.element_ty.is_fp16()) or tl.constexpr(inp.type.element_ty.is_bf16()):
        compute_dtype = tl.float32
    elif tl.constexpr(inp.type.element_ty.is_int8()) or tl.constexpr(inp.type.element_ty.is_int16()):
...
def scan_then_fan_loop(inp, out, out_indices, A, B, C, dtype):
    ...
    grid = (A, C)
    with torch_device_fn.device(inp.device):
        scan_part_min_abc_loop_kernel[grid](inp, out, out_indices, B, C, loop_num, BLOCK_SIZE)

def cummin(input: Tensor, dim: int, *, out: Union[Tensor, Tuple[Tensor, ...], List[Tensor], None]=None) -> torch.return_types.cummin:
