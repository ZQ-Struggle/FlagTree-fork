...
@triton.jit
def t_copy_2d_kernel(in_ptr, out_ptr, in_stride_0, in_stride_1, out_stride_0, out_stride_1, M, N, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr):
...
@triton.jit
def copy_1d_strided_kernel(in_ptr, out_ptr, in_stride, out_stride, N, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
...
def _launch_t_copy_kernel(inp: torch.Tensor, out: torch.Tensor):
    ...
    if dim == 0:
        ...
        copy_1d_strided_kernel[grid](inp, out, 0, 0, n, BLOCK_SIZE=1)
    elif dim == 1:
        n = inp.numel()
        ...
        grid = lambda meta: (triton.cdiv(n, meta['BLOCK_SIZE']),)
        copy_1d_strided_kernel[grid](inp, out, in_stride, out_stride, n, BLOCK_SIZE=1024)
    elif dim == 2:
        M, N = inp.shape
        assert out.dim() == 2 and out.shape[0] == N and (out.shape[1] == M), 'Output shape must be (input.size(1), input.size(0)) for t_copy'
        in_s0, in_s1 = inp.stride()
        out_s0, out_s1 = out.stride()
        grid = lambda meta: (triton.cdiv(N, meta['BLOCK_M']), triton.cdiv(M, meta['BLOCK_N']))
        t_copy_2d_kernel[grid](inp, out, in_s0, in_s1, out_s0, out_s1, M, N, BLOCK_M=32, BLOCK_N=32)
    else:
        raise RuntimeError('t_copy expects a tensor with <= 2 dims')

def t_copy_out(input: torch.Tensor, out: torch.Tensor, memory_format: torch.memory_format | None=None):
...
def t_copy(input: torch.Tensor, memory_format: torch.memory_format | None=None):
    ...
    if dim == 0:
        ...
        M, N = input.shape
        out = torch.empty((N, M), dtype=input.dtype, device=input.device, memory_format=torch.contiguous_format)
    else:
        raise RuntimeError('t_copy expects a tensor with <= 2 dims')
    _launch_t_copy_kernel(input, out)
    return out
