...
@triton.jit
def replication_pad1d_kernel(in_ptr, out_ptr, B: tl.constexpr, C: tl.constexpr, W_in, W_out, pad_left, in_stride_n, in_stride_c, in_stride_w, out_stride_n, out_stride_c, out_stride_w, BLOCK_SIZE: tl.constexpr):
...
def _launch_replication_pad1d_kernel(input: torch.Tensor, padding, out: torch.Tensor):
    ...
    if tuple(out.shape) != expected_out_shape:
        raise ValueError(f'Output tensor has incorrect shape. Expected {expected_out_shape}, got {tuple(out.shape)}')
    grid = (triton.cdiv(W_out, 256), B * C)
    with torch_device_fn.device(input.device):
        replication_pad1d_kernel[grid](input, out, B, C, W_in, W_out, left, in_s_n if dim == 3 else in_s_n, in_s_c, in_s_w, out_s_n if dim == 3 or out.dim() == 3 else 0, out_s_c, out_s_w, BLOCK_SIZE=256)
    return out
...
def replication_pad1d(input: torch.Tensor, padding):
    ...
    if input.dim() == 3:
        N, C, W_in = input.shape
        out = torch.empty((N, C, W_in + left + right), device=input.device, dtype=input.dtype, layout=input.layout)
    elif input.dim() == 2:
        C, W_in = input.shape
        out = torch.empty((C, W_in + left + right), device=input.device, dtype=input.dtype, layout=input.layout)
    else:
        raise ValueError('replication_pad1d expects 2D (C, W) or 3D (N, C, W) input')
