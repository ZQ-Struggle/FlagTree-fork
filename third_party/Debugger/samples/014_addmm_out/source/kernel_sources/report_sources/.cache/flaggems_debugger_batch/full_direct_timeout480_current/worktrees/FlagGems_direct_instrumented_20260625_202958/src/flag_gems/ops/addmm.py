...
@libentry()
@libtuner(configs=runtime.get_tuned_config('addmm'), key=['M', 'N', 'K'], strategy=['align32', 'align32', 'align32'], warmup=5, rep=10, flagtune_op_name='addmm')
@triton.jit(do_not_specialize=['alpha', 'beta'])
def addmm_kernel(a_ptr, b_ptr, i_ptr, c_ptr, alpha, beta, M, N, K, stride_am, stride_ak, stride_bk, stride_bn, stride_im, stride_in, stride_cm, stride_cn, BLOCK_SIZE_M: tl.constexpr, BLOCK_SIZE_N: tl.constexpr, BLOCK_SIZE_K: tl.constexpr, IS_FP64: tl.constexpr=False):
...
def addmm(bias, mat1, mat2, *, beta=1, alpha=1):
    ...
    with torch_device_fn.device(mat1.device):
        addmm_kernel[grid](mat1, mat2, bias, out, alpha, beta, M, N, K, mat1.stride(0), mat1.stride(1), mat2.stride(0), mat2.stride(1), bias.stride(0), bias.stride(1), out.stride(0), out.stride(1), IS_FP64=mat1.dtype == torch.float64)
    return out
...
def addmm_dtype(bias, mat1, mat2, out_dtype, *, beta=1, alpha=1):
    ...
    out = torch.empty((mat1.shape[0], mat2.shape[1]), device=mat1.device, dtype=out_dtype)
