...
@libentry()
@libtuner(configs=runtime.get_tuned_config('replication_pad3d'), key=['H_out', 'W_out'])
@triton.jit
def replicationpad3d_kernel(x_ptr, out_ptr, D_in, H_in, W_in, D_out, H_out, W_out, pad_l, pad_t, pad_f, stride_xn, stride_xc, stride_xd, stride_xh, stride_xw, stride_on, stride_oc, stride_od, stride_oh, stride_ow, C, BLOCK_H: tl.constexpr, BLOCK_W: tl.constexpr):
...
def replication_pad3d(x, padding):
    ...
    grid = lambda META: (triton.cdiv(W_out, META['BLOCK_W']), triton.cdiv(H_out, META['BLOCK_H']), N * C * D_out)
    replicationpad3d_kernel[grid](x, out, D_in, H_in, W_in, D_out, H_out, W_out, pad_l, pad_t, pad_f, *x.stride(), *out.stride(), C)
