...
def check_dtype(fill_value, dtype, device):
    ...
    elif dtype in ALL_INT_DTYPES and (fill_value < torch.iinfo(dtype).min or fill_value > torch.iinfo(dtype).max) or (dtype in ALL_FLOAT_DTYPES and (not (math.isinf(fill_value) or math.isnan(fill_value))) and (fill_value < torch.finfo(dtype).min or fill_value > torch.finfo(dtype).max)):
...
def full_kernel(out_ptr, N, fill_value, BLOCK_SIZE: tl.constexpr, SUBBLOCK_SIZE: tl.constexpr):
