...
_INT_DTYPE_BITS = {torch.bool: 1, torch.int8: 8, torch.uint8: 8, torch.int16: 16, torch.int32: 32, torch.float16: 16, torch.bfloat16: 16, torch.float32: 32}

@libentry()
@triton.jit
def _unique_dim_argsort_rank_kernel(keys_ptr: tl.tensor, indices_ptr: tl.tensor, sorted_keys_ptr: tl.tensor, num_keys: int, BLOCK_SIZE: tl.constexpr, STORE_SORTED_KEYS: tl.constexpr):
...
@libentry()
@triton.jit
def _unique_dim_gather_1d_kernel(input_ptr: tl.tensor, index_ptr: tl.tensor, output_ptr: tl.tensor, num_elements: int, BLOCK_SIZE: tl.constexpr):
...
@libentry()
@triton.jit
def _unique_dim_group_id_kernel(composite_ptr: tl.tensor, group_id_ptr: tl.tensor, last_group_id_ptr: tl.tensor, num_rows: int, BLOCK_SIZE: tl.constexpr):
    tl.debug_collect_start(level=1, addr_level=1)
...
@libentry()
@triton.jit
def _unique_dim_row_hash_chunk_kernel(flat_ptr: tl.tensor, chunk_hash_ptr: tl.tensor, num_rows: int, row_len: int, num_chunks: int, BLOCK_SIZE: tl.constexpr):
    row = ext.program_id(0)
...
@libentry()
@triton.jit
def _unique_dim_row_hash_reduce_kernel(chunk_hash_ptr: tl.tensor, row_hash_ptr: tl.tensor, num_chunks: int, BLOCK_CHUNKS: tl.constexpr):
    row = ext.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
...
@libentry()
@triton.jit
def _unique_dim_row_chunk_diff_kernel(flat_ptr: tl.tensor, sorted_indices_ptr: tl.tensor, row_chunk_diff_ptr: tl.tensor, num_rows: int, row_len: int, num_chunks: int, BLOCK_SIZE: tl.constexpr):
    row = ext.program_id(0)
    chunk = ext.program_id(1)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = chunk * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
...
@libentry()
@triton.jit
def _unique_dim_row_chunk_diff_hash_kernel(flat_ptr: tl.tensor, sorted_indices_ptr: tl.tensor, row_hash_ptr: tl.tensor, row_chunk_diff_ptr: tl.tensor, num_rows: int, row_len: int, num_chunks: int, BLOCK_SIZE: tl.constexpr):
    row = ext.program_id(0)
    chunk = ext.program_id(1)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = chunk * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
...
@libentry()
@triton.jit
def _unique_dim_row_diff_reduce_kernel(row_chunk_diff_ptr: tl.tensor, is_first_ptr: tl.tensor, num_chunks: int, BLOCK_CHUNKS: tl.constexpr):
    row = ext.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    chunks = tl.arange(0, BLOCK_CHUNKS)
    mask = chunks < num_chunks
...
@libentry()
@triton.jit
def _unique_dim_row_single_chunk_first_kernel(flat_ptr: tl.tensor, sorted_indices_ptr: tl.tensor, is_first_ptr: tl.tensor, num_rows: int, row_len: int, BLOCK_SIZE: tl.constexpr):
    row = ext.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = tl.arange(0, BLOCK_SIZE)
    mask = offsets < row_len
...
@libentry()
@triton.jit
def _unique_dim_gather_moved_kernel(flat_ptr: tl.tensor, unique_indices_ptr: tl.tensor, output_ptr: tl.tensor, num_unique: int, row_len: int, BLOCK_SIZE: tl.constexpr):
    row = ext.program_id(0)
    chunk = ext.program_id(1)
    tl.debug_collect_start(level=1, addr_level=1)
...
@libentry()
@triton.jit
def _unique_dim_inverse_permutation_kernel(sorted_indices_ptr: tl.tensor, inverse_ptr: tl.tensor, num_rows: int, BLOCK_SIZE: tl.constexpr):
    pid = ext.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offsets < num_rows
...
@libentry()
@triton.jit
def _unique_dim_inverse_kernel(sorted_indices_ptr: tl.tensor, inverse_sorted_ptr: tl.tensor, inverse_ptr: tl.tensor, num_rows: int, BLOCK_SIZE: tl.constexpr):
    pid = ext.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offsets < num_rows
...
@libentry()
@triton.jit
def _unique_dim_counts_kernel(first_positions_ptr: tl.tensor, counts_ptr: tl.tensor, num_rows: int, num_unique: int, BLOCK_SIZE: tl.constexpr):
    pid = ext.program_id(0)
    tl.debug_collect_start(level=1, addr_level=1)
    offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offsets < num_unique
    positions = tl.load(first_positions_ptr + offsets, mask=mask)
    next_positions = tl.load(first_positions_ptr + offsets + 1, mask=offsets + 1 < num_unique, other=num_rows)
...
@libentry()
@triton.jit
def _unique_dim_build_key_kernel(flat_ptr: tl.tensor, indices_ptr: tl.tensor, group_id_ptr: tl.tensor, out_ptr: tl.tensor, num_rows: int, row_stride: int, col: int, KEY_OFFSET: tl.constexpr, KEY_SCALE: tl.constexpr, REMAP_KIND: tl.constexpr, FIRST: tl.constexpr, BLOCK_SIZE: tl.constexpr):
    ...
    if REMAP_KIND == 0:
        ...
        bits = tl.load(flat_ptr + base, mask=mask, other=0).to(tl.int64) & 65535
        sign = bits & 32768 != 0
        val = tl.where(sign, bits ^ 65535, bits ^ 32768)
    else:
        bits = tl.load(flat_ptr + base, mask=mask, other=0).to(tl.int64) & 4294967295
        sign = bits & 2147483648 != 0
        val = tl.where(sign, bits ^ 4294967295, bits ^ 2147483648)
    if FIRST:
        out = val
    else:
...
def _build_composite_key(flat_view: torch.Tensor, col: int, indices: torch.Tensor | None, group_id: torch.Tensor | None, num_rows: int, row_stride: int, key_offset: int, key_scale: int, remap_kind: int) -> torch.Tensor:
    ...
    block_size = _UNIQUE_DIM_GATHER_BLOCK_SIZE if remap_kind == _REMAP_INT else _UNIQUE_DIM_BUILD_KEY_FLOAT_BLOCK_SIZE
    grid = (triton.cdiv(num_rows, block_size), 1, 1)
    with torch_device_fn.device(flat_view.device.index):
        _unique_dim_build_key_kernel[grid](flat_view, indices_arg, group_id_arg, out, num_rows, row_stride, col, KEY_OFFSET=key_offset, KEY_SCALE=key_scale, REMAP_KIND=remap_kind, FIRST=first, BLOCK_SIZE=block_size, num_warps=4)
    return out
...
def _triton_gather_1d(values: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
    ...
    with torch_device_fn.device(values.device.index):
        _unique_dim_gather_1d_kernel[grid](values, indices, output, num_elements, BLOCK_SIZE=_UNIQUE_DIM_GATHER_BLOCK_SIZE, num_warps=4)
    return output
...
def _argsort_keys(keys: torch.Tensor):
    ...
    if num_keys <= _UNIQUE_DIM_RANK_SORT_MAX_KEYS:
        ...
        with torch_device_fn.device(keys.device.index):
            _unique_dim_argsort_rank_kernel[num_keys, 1, 1](keys.contiguous(), perm, sorted_keys, num_keys, BLOCK_SIZE=block_size, STORE_SORTED_KEYS=True, num_warps=_triton_num_warps(block_size))
...
def _group_id_from_sorted(sorted_keys: torch.Tensor):
    ...
    if num_rows <= _UNIQUE_DIM_GROUP_SCAN_BLOCK_SIZE:
        ...
        with torch_device_fn.device(device.index):
            _unique_dim_group_id_kernel[1, 1, 1](sorted_keys, group_id, last_group_id, num_rows, BLOCK_SIZE=block_size, num_warps=_triton_num_warps(block_size))
        return (group_id, int(last_group_id.item()))
    diff = (sorted_keys[1:] - sorted_keys[:-1] != 0).to(torch.int64)
    group_id = torch.cat([torch.zeros(1, dtype=torch.int64, device=device), torch.cumsum(diff, dim=0)])
...
def _lex_argsort_rows_composite(flat: torch.Tensor):
    ...
    for col in range(num_cols):
        keys = _build_composite_key(flat_view, col, indices, group_id, num_rows, num_cols, key_offset, key_scale, remap_kind)
...
def _unique_dim_row_hash(flat: torch.Tensor) -> torch.Tensor:
    ...
    with torch_device_fn.device(flat.device.index):
        _unique_dim_row_hash_chunk_kernel[num_rows, num_chunks, 1](flat, chunk_hash, num_rows, row_len, num_chunks, BLOCK_SIZE=block_size, num_warps=_triton_num_warps(block_size))
        _unique_dim_row_hash_reduce_kernel[num_rows, 1, 1](chunk_hash, row_hash, num_chunks, BLOCK_CHUNKS=triton.next_power_of_2(num_chunks), num_warps=_triton_num_warps(triton.next_power_of_2(num_chunks)))
    return row_hash
...
def _unique_dim_first_mask(flat: torch.Tensor, sorted_indices: torch.Tensor):
    ...
    if num_chunks == 1:
        with torch_device_fn.device(flat.device.index):
            _unique_dim_row_single_chunk_first_kernel[num_rows, 1, 1](flat, sorted_indices, is_first, num_rows, row_len, BLOCK_SIZE=block_size, num_warps=_triton_num_warps(block_size))
        return is_first
    row_chunk_diff = torch.empty((num_rows, num_chunks), dtype=torch.int32, device=flat.device)
    grid = (num_rows, num_chunks, 1)
    row_hash = _unique_dim_row_hash(flat) if row_len >= _UNIQUE_DIM_HASH_MIN_ROW_LEN else None
    with torch_device_fn.device(flat.device.index):
        if row_hash is None:
            _unique_dim_row_chunk_diff_kernel[grid](flat, sorted_indices, row_chunk_diff, num_rows, row_len, num_chunks, BLOCK_SIZE=block_size, num_warps=_triton_num_warps(block_size))
        else:
            _unique_dim_row_chunk_diff_hash_kernel[grid](flat, sorted_indices, row_hash, row_chunk_diff, num_rows, row_len, num_chunks, BLOCK_SIZE=block_size, num_warps=_triton_num_warps(block_size))
        _unique_dim_row_diff_reduce_kernel[num_rows, 1, 1](row_chunk_diff, is_first, num_chunks, BLOCK_CHUNKS=triton.next_power_of_2(num_chunks), num_warps=_triton_num_warps(triton.next_power_of_2(num_chunks)))
    return is_first

def _unique_dim_gather_output(moved: torch.Tensor, unique_indices: torch.Tensor, dim: int, input_shape: torch.Size) -> torch.Tensor:
    num_unique = unique_indices.numel()
    output_shape = tuple(input_shape[:dim]) + (num_unique,) + tuple(input_shape[dim + 1:])
    if num_unique == 0:
    ...
    moved_output = torch.empty((num_unique,) + tuple(moved.shape[1:]), dtype=moved.dtype, device=moved.device)
    ...
    with torch_device_fn.device(moved.device.index):
        _unique_dim_gather_moved_kernel[grid](flat, unique_indices, moved_output, num_unique, row_len, BLOCK_SIZE=_UNIQUE_DIM_GATHER_BLOCK_SIZE, num_warps=4)
...
def _unique_dim_inverse_from_permutation(sorted_indices: torch.Tensor) -> torch.Tensor:
    ...
    with torch_device_fn.device(sorted_indices.device.index):
        _unique_dim_inverse_permutation_kernel[grid](sorted_indices, inverse_indices, num_rows, BLOCK_SIZE=_UNIQUE_DIM_GATHER_BLOCK_SIZE, num_warps=4)
    return inverse_indices

def _unique_dim_inverse(sorted_indices: torch.Tensor, is_first: torch.Tensor) -> torch.Tensor:
    """Inverse mapping: scatter dense group ids back to original positions."""
    num_rows = sorted_indices.numel()
    inverse_indices = torch.empty(num_rows, dtype=torch.int64, device=sorted_indices.device)
    if num_rows == 0:
        return inverse_indices
    inverse_in_sorted = torch.cumsum(is_first.to(torch.int64), dim=0) - 1
    grid = (triton.cdiv(num_rows, _UNIQUE_DIM_GATHER_BLOCK_SIZE), 1, 1)
    with torch_device_fn.device(sorted_indices.device.index):
        _unique_dim_inverse_kernel[grid](sorted_indices, inverse_in_sorted, inverse_indices, num_rows, BLOCK_SIZE=_UNIQUE_DIM_GATHER_BLOCK_SIZE, num_warps=4)
    return inverse_indices

def _unique_dim_unique_indices(sorted_indices: torch.Tensor, is_first: torch.Tensor) -> torch.Tensor:
...
def _unique_dim_unique_indices_and_inverse(sorted_indices: torch.Tensor, is_first: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
...
def _unique_dim_counts(is_first: torch.Tensor, num_rows: int) -> torch.Tensor:
    ...
    with torch_device_fn.device(is_first.device.index):
        _unique_dim_counts_kernel[grid](first_positions, counts, num_rows, num_unique, BLOCK_SIZE=_UNIQUE_DIM_GATHER_BLOCK_SIZE, num_warps=4)
    return counts

def unique_dim(input: torch.Tensor, dim: int, sorted: bool=True, return_inverse: bool=False, return_counts: bool=False):
    ...
    if return_inverse:
        unique_in_orig, inverse_indices = _unique_dim_unique_indices_and_inverse(sorted_indices, is_first)
    else:
