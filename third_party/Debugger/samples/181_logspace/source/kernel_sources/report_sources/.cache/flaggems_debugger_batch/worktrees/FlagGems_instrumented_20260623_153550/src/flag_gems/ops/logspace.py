...
@libentry()
@triton.jit
def logspace_kernel(out_ptr, out_stride0, start, step_size, steps, log2_base: tl.constexpr, BLOCK_SIZE: tl.constexpr):
...
def logspace(start, end, steps, base=10.0, *, dtype=None, layout=None, device=None, pin_memory=None) -> torch.Tensor:
    ...
    out = torch.empty(steps, dtype=out_dtype, layout=layout, device=device, pin_memory=pin_memory)
    if steps == 0:
        pass
    elif steps == 1:
        ...
        logspace_kernel[grid](out, out.stride(0), start, step_size, steps, log2_base=math.log2(float(base)), BLOCK_SIZE=BLOCK_SIZE)
    return out
